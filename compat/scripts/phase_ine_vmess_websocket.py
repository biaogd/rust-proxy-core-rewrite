#!/usr/bin/env python3
"""IN-E Go/Rust differential for VMess WebSocket (WSS) inbound TCP.

Named `type: vmess` listeners with `ws-path` accept TLS + WebSocket upgrade,
then VMess auth/relay. Product VMess outbound (`network: ws`, alterId 0,
...
FAILURE_ARTIFACT = ROOT / "compat" / "artifacts" / "phase-ine-vmess-websocket-diff.json"
...
def wait_tcp_route(process: Any, mixed_port: int, echo_port: int) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            pass
        time.sleep(0.02)
    raise TimeoutError("VMess WSS inbound route did not become ready")
...
def exercise(binary: pathlib.Path, scratch: pathlib.Path) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            tunnel.sendall(b"ine-wss")
            small = recv_exact(tunnel, 7) == b"ine-wss"
        finally:
            tunnel.close()
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase-ine-vmess-wss-") as temporary:
        root = pathlib.Path(temporary)
        binaries = build_binaries(root, "PHASE_INE_VMESS_WS_CARGO_TARGET", "phase-ine-vmess-ws")
        try:
            for name in ["rust", "go"]:
                scratch = root / name
                scratch.mkdir()
