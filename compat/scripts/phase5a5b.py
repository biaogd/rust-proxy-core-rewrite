#!/usr/bin/env python3
"""Go/Rust differential gate for Phase 5A5b IP-CIDR MRS encoding."""
...
FAILURE_ARTIFACT = ROOT / "compat" / "artifacts" / "phase5a5b-diff.json"
...
def build_binaries(output: pathlib.Path) -> dict[str, pathlib.Path]:
    ...
    target = cargo_target_path("PHASE5A5B_CARGO_TARGET", "phase5a5b-rust")
    subprocess.run(
...
def decode(
    ...
    result = run(
        binary,
        ["convert-ruleset", "ipcidr", "mrs", str(source), str(target)],
        scratch,
    )
    return {
        "exit-code": result.returncode,
...
def empty_case(
    ...
    result = run(
        binary,
        ["convert-ruleset", "ipcidr", "text", str(source), str(target)],
        scratch,
    )
    return {
        "exit-code": result.returncode,
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mihomo-phase5a5b-") as temporary:
        ...
        if mismatch:
            ...
            raise SystemExit(f"Phase 5A5b mismatch; see {FAILURE_ARTIFACT}")

    FAILURE_ARTIFACT.unlink(missing_ok=True)
    print("Phase 5A5b IP-CIDR text/YAML encoding differential passed")
