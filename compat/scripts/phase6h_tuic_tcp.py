#!/usr/bin/env python3
"""Go/Rust differential for 6H-A TUIC v5 outbound TCP (Go TUIC inbound authority)."""
...
FAILURE_ARTIFACT = ROOT / "compat" / "artifacts" / "phase6h-tuic-tcp-diff.json"
PASSWORD = "phase-6ha-password"
...
def exercise(
    ...
    provider.write_text("proxies:\n" + tuic_record("provider-tuic", authority_port))
...
ipv6: true
hosts:
  echo.tuic.test: 127.0.0.1
proxies:
{tuic_record("inline-tuic", authority_port)}
{tuic_record("tuic-wrong-password", authority_port, password="wrong-password")}
proxy-providers:
  local-tuic:
    type: file
    path: {provider}
proxy-groups:
  ...
  - name: tuic-health
    ...
    tolerance: 50
rules:
  - DST-PORT,{wrong_rule_port},tuic-wrong-password
  - MATCH,tuic-select
"""
    )
    ...
    try:
        ...
        domain_small = wait_exchange(
            process, mixed_port, "echo.tuic.test", echo_port, b"tuic-domain"
        )
        ipv4_large = False
        for _ in range(5):
            try:
        ...
        if echo6_port is not None:
            ipv6_ok = exchange(mixed_port, "::1", echo6_port, b"tuic-ipv6")
        ...
        reuse_second = exchange(mixed_port, "127.0.0.1", echo_port, b"tuic-reuse")
        ...
        inline_snapshot = proxy_snapshot(controller_port, "inline-tuic")
        provider_snapshot = proxy_snapshot(
            controller_port, "provider-tuic", "local-tuic"
        )

        health_query = f"url=http://127.0.0.1:{health_port}/&timeout=5000"
        health_status, health_body = request(
            controller_port, "GET", f"/group/tuic-health/delay?{health_query}"
        )
        health_ok = health_status == 200 and b"inline-tuic" in health_body

        reload_via_controller(process, controller_port, config, secret=SECRET)
        after_reload = wait_exchange(
            process, mixed_port, "echo.tuic.test", echo_port, b"after-reload"
        )
...
ipv6: false
proxies:
{tuic_record("bad-cert-tuic", authority_port, skip_verify=False)}
rules:
  - MATCH,bad-cert-tuic
"""
        )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase-6ha-") as temporary:
        root = pathlib.Path(temporary)
        binaries = build_binaries(root, "PHASE6HTUIC_CARGO_TARGET", "phase-6ha")
        try:
            for name in ["rust", "go"]:
                scratch = root / name
                scratch.mkdir()
            ...
            observations["rust-deferred-v4-rejected"] = not config_validation(
                binaries["rust"],
                root / "rust-validate",
                "proxies:\n"
                "  - name: deferred\n"
            ...
            observations["rust-deferred-0rtt-rejected"] = not config_validation(
                binaries["rust"],
                root / "rust-validate-0rtt",
                "proxies:\n"
                "  - name: deferred\n"
                "    type: tuic\n"
                "    server: 127.0.0.1\n"
    ...
    if (
        go != rust
        or not observations.get("rust-deferred-v4-rejected", False)
        or not observations.get("rust-deferred-0rtt-rejected", False)
    ):
        FAILURE_ARTIFACT.parent.mkdir(parents=True, exist_ok=True)
        FAILURE_ARTIFACT.write_text(
            json.dumps(
                {
                    "go": go,
                    "rust": rust,
                    "rust-deferred-v4-rejected": observations.get(
                        "rust-deferred-v4-rejected"
                    ),
                    "rust-deferred-0rtt-rejected": observations.get(
                        "rust-deferred-0rtt-rejected"
                    ),
                },
                indent=2,
                sort_keys=True,
    ...
    print("6H-A TUIC v5 TCP differential passed")
