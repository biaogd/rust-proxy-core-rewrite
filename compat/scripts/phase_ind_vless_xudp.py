#!/usr/bin/env python3
"""IN-D Go/Rust differential for VLESS TLS named inbound XUDP (Mux).

Product VLESS outbound defaults to XUDP (`packet-encoding: xudp` / command
Mux). This gate proves Go and Rust named `type: vless` TLS inbounds accept that
Mux association and relay multi-destination UDP via SOCKS on the product's own
outbound client (the same style `phase_ind_vless_tls.py` uses for TCP product
round-trips).
...
FAILURE_ARTIFACT = ROOT / "compat" / "artifacts" / "phase-ind-vless-xudp-diff.json"
UUID = "b831381d-6324-4d53-ad4f-8cda48b30811"
SNI = "dot.phase4.test"
PAYLOAD_SMALL = b"ind-vless-xudp"
...
def wait_udp_route(
    ...
    # Match phase6e_vless_udp: short per-attempt timeouts so a cold first
    # datagram can be retried within the outer deadline.
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            client.close()
        time.sleep(0.02)
    raise TimeoutError("VLESS XUDP SOCKS UDP route did not become ready")
...
def exercise(binary: pathlib.Path, scratch: pathlib.Path) -> dict[str, Any]:
    ...
    server_cfg.write_text(inbound_yaml(vless_port, certificate, private_key))
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase-ind-vless-xudp-") as temporary:
        root = pathlib.Path(temporary)
        binaries = build_binaries(root, "PHASE_IND_VLESS_XUDP_CARGO_TARGET", "phase-ind-vless-xudp")
        try:
            for name in ["rust", "go"]:
                scratch = root / name
                scratch.mkdir()
