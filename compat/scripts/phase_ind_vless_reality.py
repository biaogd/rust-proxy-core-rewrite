#!/usr/bin/env python3
"""IN-D Go/Rust differential for VLESS REALITY named inbound (native TCP).

Both products expose a named `type: vless` listener with `reality-config` (Go:
...
FAILURE_ARTIFACT = ROOT / "compat" / "artifacts" / "phase-ind-vless-reality-diff.json"
...
def wait_reality_route(
    ...
    while time.monotonic() < deadline:
        ...
        if product_round_trip(
            ...
            return
        time.sleep(0.05)
    raise TimeoutError("VLESS REALITY inbound route did not become ready")
...
def exercise(binary: pathlib.Path, scratch: pathlib.Path) -> dict[str, Any]:
    ...
    try:
        wait_reality_route(binary, process, scratch, vless_port, tcp_port)
        small = product_round_trip(
            binary, scratch, vless_port, tcp_port, b"ind-vless-reality", label="small"
        )
        ...
        half_close = product_round_trip(
            ...
            vless_port,
            tcp_port,
            b"ind-vless-reality-half-close",
            half_close=True,
            label="half-close",
        )
        return {
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase-ind-vless-reality-") as temporary:
        root = pathlib.Path(temporary)
        binaries = build_binaries(root, "PHASE_IND_VLESS_REALITY_CARGO_TARGET", "phase-ind-vless-reality")
        try:
            for name in ["rust", "go"]:
                scratch = root / name
                scratch.mkdir()
        ...
        if rust_view != go_view:
            ...
            raise SystemExit("IN-D VLESS REALITY Go/Rust differential mismatch")
        ...
        if not all(
            ...
            raise SystemExit("IN-D VLESS REALITY evidence failed")

    print(json.dumps({"phase-ind-vless-reality": rust_view}, indent=2, sort_keys=True))
    return 0
