#!/usr/bin/env python3
"""IN-C / W3.1 Go/Rust differential for Trojan REALITY named inbound (native TCP).

Both products expose a named `type: trojan` listener with `reality-config` (Go:
`listener/inbound/reality.go` + trojan server; Rust: `named_listeners` +
`accept_reality`). Product Trojan outbound with `reality-opts` dials each named
...
FAILURE_ARTIFACT = ROOT / "compat" / "artifacts" / "phase-inc-trojan-reality-diff.json"
PASSWORD = "phase-inc-trojan-reality-password"
...
def wait_reality_route(
    ...
    wait_ready(process, trojan_port)
    deadline = time.monotonic() + IO_DEADLINE
    while time.monotonic() < deadline:
        if process.poll() is not None:
        ...
        if product_round_trip(
            ...
            return
        time.sleep(0.05)
    raise TimeoutError("Trojan REALITY inbound route did not become ready")
...
def exercise(binary: pathlib.Path, scratch: pathlib.Path) -> dict[str, Any]:
    ...
    config.write_text(inbound_yaml(trojan_port))
    process, stdout, stderr = launch(binary, config, scratch)
    try:
        wait_reality_route(binary, process, scratch, trojan_port, tcp_port)
        small = product_round_trip(
            binary, scratch, trojan_port, tcp_port, b"inc-trojan-reality", label="small"
        )
        large = product_round_trip(
            binary, scratch, trojan_port, tcp_port, LARGE_PAYLOAD, label="large"
        )
        half_close = product_round_trip(
            ...
            trojan_port,
            tcp_port,
            b"inc-trojan-reality-half-close",
            half_close=True,
            label="half-close",
        )
        return {
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="phase-inc-trojan-reality-") as temporary:
        root = pathlib.Path(temporary)
        binaries = build_binaries(
            root, "PHASE_INC_TROJAN_REALITY_CARGO_TARGET", "phase-inc-trojan-reality"
        )
        try:
            for name in ["rust", "go"]:
                scratch = root / name
        ...
        if rust_view != go_view:
            ...
            raise SystemExit("IN-C Trojan REALITY Go/Rust differential mismatch")
        ...
        if not all(
            ...
            raise SystemExit("IN-C Trojan REALITY evidence failed")

    print(json.dumps({"phase-inc-trojan-reality": rust_view}, indent=2, sort_keys=True))
    return 0
