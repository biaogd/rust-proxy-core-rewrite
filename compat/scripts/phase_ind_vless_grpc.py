...
def wait_tcp_route(process: Any, mixed_port: int, echo_port: int) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            pass
        time.sleep(0.02)
    raise TimeoutError("VLESS gRPC inbound route did not become ready")
        ...
        try:
            ...
            if not observations["rust"]["config"].get("reject-ws-plus-grpc"):
                raise AssertionError(
                    "Rust IN-D must reject combined VLESS ws-path + grpc-service-name"
                )
