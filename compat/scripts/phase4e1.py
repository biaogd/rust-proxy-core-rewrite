#!/usr/bin/env python3
"""Local Go/Rust differential suite for the Phase 4E1 DoT gate."""
...
FIXTURE = ROOT / "compat" / "fixtures" / "phase4" / "dot.yaml.tmpl"
CERTIFICATE = ROOT / "test" / "config" / "example.org.pem"
PRIVATE_KEY = ROOT / "test" / "config" / "example.org-key.pem"
FAILURE_ARTIFACT = ROOT / "compat" / "artifacts" / "phase4e1-diff.json"
...
def exercise(binary: pathlib.Path, scratch: pathlib.Path) -> dict[str, Any]:
    ...
    try:
        ...
        for inbound, query_fn, identifier in (
            ("udp", udp_query, 0x7E10),
            ("tcp", tcp_query, 0x7E20),
        ):
            name = f"{inbound}.dot.phase4.test"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mihomo-phase4e1-") as temporary:
        ...
        if observations["go"] != observations["rust"]:
            ...
            raise SystemExit(f"Phase 4E1 mismatch; see {FAILURE_ARTIFACT}")
    FAILURE_ARTIFACT.unlink(missing_ok=True)
    print("Phase 4E1 Go/Rust differential suite passed")
