...
impl TlsConnector {
    ...
    #[inline]
    pub fn connect_with_session_id_generator<IO>(
        &self,
    ...
    where
        IO: AsyncRead + AsyncWrite + Unpin,
    {
        let alpn = self.inner.alpn_protocols.clone();
        let session = match ClientConnection::new_with_alpn_and_session_id_generator(
            self.inner.clone(),
            domain,
            alpn,
