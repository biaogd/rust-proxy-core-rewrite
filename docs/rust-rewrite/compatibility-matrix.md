...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–L adds VMess security/AlterID/packet modes plus TLS, WS, HTTP, H2, pooled gRPC/Gun, mKCP and Mekya TCP transports. K/L accept typed `mkcp-opts`/`mekya-opts` with misplaced/negative/invalid-URL rejection. Phase 6E-A/B adds VLESS v0 native TCP, canonical/Go-mapped UUIDs and verified/skip TLS naming policy. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VLESS | Oracle | **Parity in Phase 6E-A–H declared client scopes** | A/B cover v0 native TCP/TLS. C–E add WS/WSS, HTTP/H2 and single-stream gRPC/Gun. F adds plaintext native-TCP UDP with packet-address and source-stable XUDP; UDP-over-TLS remains pending differential evidence. G proves Vision raw DIRECT with nested TLS. H proves REALITY default/hybrid handshake and normalized Chrome 133 ClientHello semantics. Encryption extensions, xHTTP, pooled gRPC, other fingerprints, legacy-only TLS 1.2 suites, Vision+REALITY, mux and server direction remain open |
...
| Shared outbound transport/security variants | Oracle | Partial | Phase 6D-F–J covers TLS/WS/Upgrade/HTTP/H2 and pooled Gun. Phase 6D-K adds V2Ray mKCP segment/auth/header, exact Go congestion/RTO/close-state contracts and deterministic moderate/heavy semantic impairment; 6D-L layers mKCP over Hyper-backed Mekya HTTP/1.1/H2 and matches local half-close plus peer-close outcomes. Current K/L evidence is VMess TCP on deterministic loopback; xHTTP/H3, randomized long-duration impairment, other consumers, Reality, ECH, JLS, ReSTLS, ShadowTLS and TLSMirror remain separate gates |
...
| Darwin arm64 — Phase 6E-F VLESS UDP/XUDP | Oracle | **Parity in declared plaintext scope; TLS pending** | Native differential covers packet-address and XUDP multi-destination associations, controller capabilities and process survival. TLS carrier composition and source-stable XUDP identity are implemented, but UDP-over-TLS still needs Go/Rust differential evidence, 2026-09-02 |
...
| Linux amd64 — Phase 6E-C–F VLESS transports and UDP | Oracle | Pending | Default controller/outbound shards run the WS/WSS, HTTP/H2, gRPC/Gun and UDP/XUDP differentials; no native result is claimed before completion |
| Linux amd64 — Phase 6E-G/H Vision and REALITY | Oracle | Pending | Default CI runs nested-TLS Vision DIRECT plus REALITY default/hybrid and normalized Chrome 133 ClientHello differentials; no native Linux result is claimed before completion |
