...
in progress (8A Linux + 8B Darwin arm64)**; see [roadmap](roadmap.md#next-work-priority--2026-09-09).
...
| TUN and route settings | Oracle | **Partial** (8A Linux + 8B Darwin apply; native Parity unclaimed) | Common keys parse; `stack: smoltcp` accepted; Go `system`/`gvisor`/`mixed` rejected without remap. Darwin `tun.device` must be empty or `utunN` (no remap of `tun0`/`utun`). Unprivileged evidence: config unit tests and `compat/scripts/phase8a_tun.py` / `phase8b_tun.py`. Privileged Linux netns fixture covers manual then auto-route, DNS hijack/fake-IP HTTP, large transfer, UDP echo, init-failure rollback and stop cleanup (`PHASE8A_NATIVE=1`, fail-closed). Privileged Darwin arm64 fixture covers utun, scutil DNS rewrite/restore, split auto-route, HTTP/UDP through TUN and invalid-device leftover (`PHASE8B_NATIVE=1`, fail-closed). No Parity until those jobs report success |
...
| TUN | Yes | Yes | Oracle | **Partial** (Linux 8A + Darwin 8B data path; native pass unclaimed until CI) | Parse + stack identity: `compat/scripts/phase8a_tun.py` / `phase8b_tun.py` and config unit tests (`tun-rs` 2.8.9 + `netstack-smoltcp` 0.2.4; `smoltcp` only). Runtime wires TUN TCP/UDP into existing rules/outbound, DNS hijack, owned auto-route and stop/failure cleanup on Linux and Darwin arm64. Darwin adds utun, split default prefixes, and scutil DNS ownership/restore. Privileged Linux netns HTTP/DNS/UDP echo, fake-IP reverse mapping, Go `system` vs Rust `smoltcp` body match, init-failure leftover check and stop cleanup are implemented and fail-closed (`PHASE8A_NATIVE=1`). Privileged Darwin arm64 HTTP/UDP/system-DNS/auto-route/cleanup is fail-closed (`PHASE8B_NATIVE=1`). Parity only after those jobs report success. 8C Windows follows; 8F before 8D/8E |
...
| DNS hijack through TUN | Oracle | **Partial** (8A netns + 8B Darwin system DNS) | `compat/scripts/phase8a_tun.py` with `PHASE8A_NATIVE=1` sends UDP/53 through the TUN device, requires a fake-IP in `198.19.0.0/16`, then reverse-maps DOMAIN rules for HTTP/UDP. Darwin 8B also rewrites the primary service DNS via `scutil` to the TUN next address and restores it on stop (`PHASE8B_NATIVE=1`). Loop-avoidance covers literal DNS/proxy IPs only. No Parity until the privileged jobs report success |
