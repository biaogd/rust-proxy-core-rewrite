...
| Phase 3 local rule/listener reload | Oracle | **Parity** | Same-port rule switch, invalid-config rollback and port migration through the portable `/configs` transaction in `compat/scripts/phase3.py`; Unix SIGHUP source semantics remain covered separately |
...
| Cross-platform and future-resource hook ordering | Oracle | Partial | Windows uses `cmd.exe /C` and the native differential now drives a real `CTRL_BREAK_EVENT`; the next native matrix result is still pending, and every future runtime resource must join the readiness/shutdown barrier |
