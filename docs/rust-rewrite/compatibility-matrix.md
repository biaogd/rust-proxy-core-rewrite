...
| Shadowsocks (`ss`) | Oracle | **Partial — Phase 6C-M6 client wire parity and 6C-N first-server parity in their declared scopes** | A–H proves eight shared SIP004 AEAD plus eight legacy stream methods over TCP/native UDP, providers/health and UoT v1/v2. I/J proves three standard 2022 methods and AES single-hop EIH over TCP; K adds ChaCha8-2022 TCP. L proves shared native UDP relay to explicit IPv6 destinations. M1/M2 adds simple-obfs HTTP/TLS; M5 closes documented v2ray-plugin TCP options; M6 adds native `plugin: shadow-tls` v1/v2/v3 over SS2022 TCP. M6 Chrome advertises all 16 Go/uTLS suites, but its wrapper-specific extension regression remains per-runtime rather than wire parity. Phase 6C-N separately validates the first inbound/server slice. Native 2022 UDP, multi-hop EIH, Go-only methods, advanced socket/dialer options and inbound features outside 6C-N remain open |
...
| Darwin arm64 — Phase 6C-I Shadowsocks 2022 TCP | Oracle | **Parity in declared scope** | Native three-method single-PSK config and official-library TCP differential passed for domain/large/half-close relay, 2026-08-29; UDP is disabled after a pinned-Go interop panic; EIH/extra methods remain open |
| Darwin arm64 — Phase 6C-J Shadowsocks 2022 EIH | Oracle | **Parity in declared scope** | Native AES-128/256 single-hop EIH config and official-library TCP differential passed for domain/large/half-close relay, 2026-08-29; multi-hop EIH, 2022 UDP and extra methods remain open |
| Darwin arm64 — Phase 6C-K Shadowsocks 2022 ChaCha8 TCP | Oracle | **Parity in declared scope** | Native exact-key config and official-library TCP differential passed for domain/large/half-close relay, 2026-08-29; 2022 UDP remains blocked |
