...
| Proxy providers and health checks | Oracle | Partial | Local YAML file load, views, selected routing, manual transactional refresh/rollback and filtered composition pass. A plaintext HTTP vehicle with explicit cache path now passes bounded initial/manual/interval fetch, validated atomic cache replacement, group switching, 503 config/cache rollback and REST views; HTTPS, optional hashed paths, headers, ETag/conditional fetch and real provider health remain |
...
| Proxy and rule provider APIs | Oracle | Partial | File provider views/composition pass; `phase5c_provider_refresh.py` proves PUT replacement and rollback. Phase 5C2c/2d add plaintext HTTP vehicle/detail/member views, initial and remote PUT/interval refresh, selected replacement routing, validated atomic cache replacement, malformed/status 503 and full old-state rollback; HTTPS, ETag, concurrent/coalesced refresh, real health state, rule-provider detail and broader persistence remain unclaimed |
