...
| VLESS | Oracle | **Parity in Phase 6E-A/B declared native-TCP client scope** | Phase A covers v0 framing, canonical/Go-mapped UUIDs, all address types, lazy payload, addons, integration and half-close. Phase B adds native TCP over rustls with trusted roots, SNI, independent verification name, skip verification, negative trust/name cases and controller health in `phase6e_vless_tls.py`; UDP/packet modes, encryption extensions, non-native transports, advanced TLS, Vision/Reality, mux and server direction remain open |
