...
| Proxy and rule provider APIs | Oracle | Partial | Implicit/default/configured-compatible provider contracts pass; `phase5c_file_provider.py` adds exact File provider list/detail/member/health JSON, fixed-mtime observation and group-compatible provider registration. Refresh mutation, HTTP vehicles, real health state, rule-provider detail and persistence remain unclaimed |
