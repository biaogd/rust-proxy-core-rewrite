...
| TUN and route settings | Oracle | **Partial** (8A parse + Linux apply + netns harness) | Common keys parse; `stack: smoltcp` accepted; Go `system`/`gvisor`/`mixed` rejected without remap. Unprivileged evidence: config unit tests and `compat/scripts/phase8a_tun.py`. Privileged Linux netns fixture covers manual then auto-route, DNS hijack/fake-IP HTTP, large transfer, UDP echo, init-failure rollback and stop cleanup (`PHASE8A_NATIVE=1`, fail-closed). No Parity until that job reports success |
...
| TUN | Yes | Yes | Oracle | **Partial** (Linux 8A data path + netns harness; native pass unclaimed until CI) | Parse + stack identity: `compat/scripts/phase8a_tun.py` and config unit tests (`tun-rs` 2.8.9 + `netstack-smoltcp` 0.2.4; `smoltcp` only). Runtime wires TUN TCP/UDP into existing rules/outbound, DNS hijack, owned auto-route and stop/failure cleanup. Privileged Linux netns HTTP/DNS/UDP echo, fake-IP reverse mapping, Go `system` vs Rust `smoltcp` body match, init-failure leftover check and stop cleanup are implemented and fail-closed (`PHASE8A_NATIVE=1`). Parity only after that job reports success. 8B macOS arm64 / 8C Windows follow; 8F before 8D/8E |
...
| DNS hijack through TUN | Oracle | **Partial** (8A netns harness) | `compat/scripts/phase8a_tun.py` with `PHASE8A_NATIVE=1` sends UDP/53 through the TUN device, requires a fake-IP in `198.19.0.0/16`, then reverse-maps DOMAIN rules for HTTP/UDP. Loop-avoidance covers literal DNS/proxy IPs only. No Parity until the privileged job reports success |
