...
| Proxy groups and cycle/order validation | Oracle | Partial | Flat select membership plus ordered filter/exclude, all three include-all forms and empty fallback pass; nested DAG/cycles, automatic groups and exhaustive validation remain |
...
| Proxy groups/select/fallback/url-test/load-balance | Oracle | Partial | Flat selector control/reload passes; `phase5c_file_provider.py` adds ordered `use` composition from one local provider, manual provider-member selection and live HTTP TCP routing. Nested groups, filters/include-all, restart persistence and automatic strategies remain unclaimed |
...
| `/proxies`, `/group`, delay and selection | Oracle | Partial | `phase5d_proxies.py` proves all seven built-ins and GLOBAL; Phase 5C adds configured selectors, reload reconciliation, filtered/include-all provider composition, empty fallback and live selection-driven routing; selector fixtures isolate HOME/XDG/Mihomo profile state so persisted choices cannot cross scenarios; automatic/nested groups, HTTPS and exhaustive failure/persistence remain unclaimed |
...
| Proxy and rule provider APIs | Oracle | Partial | File provider views/composition pass; `phase5c_provider_refresh.py` proves PUT replacement, old/new member visibility, selected new-member routing, malformed-YAML 503 and full old-state rollback. HTTP/interval vehicles, concurrent refresh, real health state, rule-provider detail and persistence remain unclaimed |
