...
| Windows amd64 — full Rust workspace build | Oracle | Build passed | Actions run `33158824630` asserted the native X64 runner and built the locked workspace with all targets/features; Windows keeps profile state in memory because `bbolt-rs` 1.3.10 is excluded there |
