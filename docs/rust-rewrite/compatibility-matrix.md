...
| Snell | Yes | Yes | Oracle | Partial — 7E-A outbound TCP only | Versions 1–3 TCP client; inbound/UDP/pool/obfs remain rejected |
...
| Snell | Oracle | **Partial — 7E-A versions 1–3 TCP outbound** | YAML `type: snell` `psk` + version 1/2/3 through mixed/SOCKS TCP, rules/groups/providers/health/reload in `phase7e_snell_tcp.py`. v1 ChaCha20-Poly1305, v2/v3 AES-128-GCM, Argon2id KDF. `udp`/`reuse`/`obfs-opts`/v4/v5/`dialer-proxy` and inbound remain rejected. Not Parity; three-platform CI is the new snell shard |
