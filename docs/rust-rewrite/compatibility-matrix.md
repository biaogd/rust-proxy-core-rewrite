...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–E adds top-level and file-provider/selector VMess records with fail-closed validation for every native-TCP security mode, positive legacy AlterID and standard/packet-address/XUDP UDP selection. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VMess | Oracle | **Parity in Phase 6D-A–E native-TCP client scope** | Every supported security mode passes independent-authority differentials with AEAD and positive legacy AlterID headers. Phase 6D-E additionally proves ordinary fixed-destination UDP, packet-address IPv4/IPv6 and XUDP multi-destination framing through the existing mixed/SOCKS5 UDP ingress, including association reuse and controller `udp`/`uot`/`xudp` shape. Negative AlterID, oversized UDP edges, packet-address FQDN payloads, TLS, outer transports, general TCP mux and server direction remain open |
