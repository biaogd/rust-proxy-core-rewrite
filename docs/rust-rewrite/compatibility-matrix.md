...
| LAN allow/deny and skip-auth prefixes | Oracle | **Parity in declared fixed-listener scope** | `phase5f_lan_policy.py` proves IPv4/IPv6 wildcard and explicit bind, same-port IPv4→IPv6 PATCH rebind, allow-before-deny filtering, auth bypass, malformed-prefix rollback, loopback override and `/configs` fields across fixed HTTP/SOCKS/mixed; native non-loopback reachability and TFO/MPTCP remain |
