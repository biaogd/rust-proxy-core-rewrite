...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-H covers top-level plus inline/file/HTTP-provider `ss` with eight shared SIP004 AEAD and eight shared legacy stream ciphers, native UDP, UoT v1/v2 and controller views. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Shadowsocks (`ss`) | Oracle | **Parity in Phase 6C-H declared cipher/UoT scope** | Phase 6C A–C proves three core AEAD methods; D/E adds providers and health; F proves UoT v1/v2; G proves eight legacy stream methods; H proves five extra AEAD methods. Every added cipher has TCP/native-UDP wire evidence. Go-only methods, 2022, plugins, server direction, IPv6 UDP, UoT connect mode, provider scheduling/ETag stress and advanced socket/dialer options remain open |
