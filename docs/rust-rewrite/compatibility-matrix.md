...
| Proxy providers and health checks | Oracle | Partial | Local YAML file load, views, selected routing, manual transactional refresh/rollback and filtered composition pass. A plaintext HTTP vehicle with explicit cache path now passes initial bounded fetch, validated atomic cache write, group consumption and REST views; HTTPS, optional hashed paths, headers, intervals/ETag and real provider health remain |
...
| Proxy and rule provider APIs | Oracle | Partial | File provider views/composition pass; `phase5c_provider_refresh.py` proves PUT replacement, old/new member visibility, selected new-member routing, malformed-YAML 503 and full old-state rollback. Phase 5C2c adds the initial plaintext HTTP vehicle/detail/member view and cached group routing; remote PUT, intervals/ETag, concurrent refresh, real health state, rule-provider detail and broader persistence remain unclaimed |
