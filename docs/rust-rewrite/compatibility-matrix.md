...
| Linux amd64 — Phase 8F TUN network-change | Oracle | **Partial** (monitor implemented; native pass unclaimed until CI) | Default-route poll excluding TUN, host-route re-protect, resolver `reset_connections()` without fake-IP flush, 4096 TCP/UDP caps. Privileged dual-uplink `PHASE8F_NATIVE=1` / `phase8f-linux-tun` is fail-closed. Native Darwin/Windows NIC flap, UDP fragment/loss and TUN TCP half-close/RST are not in this gate. Not Parity |
