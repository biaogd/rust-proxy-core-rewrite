...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-F covers top-level plus inline/file/HTTP-provider `ss` with the three standard SIP004 AEAD ciphers, native UDP, UoT v1/v2 and controller views. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Shadowsocks (`ss`) | Oracle | **Parity in Phase 6C-F native SIP004 AEAD scope** | Phase 6C A–C proves all three accepted ciphers over TCP/native UDP; D/E adds provider selection, HTTP lifecycle/cache and health. `phase6c_shadowsocks_uot.py` proves Go-compatible version validation, controller capability, v1/v2 encrypted TCP framing and repeated IPv4/domain-originated UDP relay. Stream/extra/2022 ciphers, plugins, server direction, IPv6 UDP, UoT connect mode, provider scheduling/ETag stress and advanced socket/dialer options remain open |
