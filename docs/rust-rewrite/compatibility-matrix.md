...
| TUN and route settings | Oracle | **Partial** (8A parse + Linux apply) | Common keys parse; `stack: smoltcp` accepted; Go `system`/`gvisor`/`mixed` rejected without remap. Unprivileged evidence: config unit tests and `compat/scripts/phase8a_tun.py`. Linux device/stack/DNS-hijack/auto-route/rollback is implemented; privileged netns traffic and route-cleanup fixtures remain the native gate (`PHASE8A_NATIVE=1`). No Parity until those fixtures pass |
...
| TUN | Yes | Yes | Oracle | **Partial** (Linux 8A data path implemented; native traffic unclaimed) | Parse + stack identity: `compat/scripts/phase8a_tun.py` and config unit tests (`tun-rs` 2.8.9 + `netstack-smoltcp` 0.2.4; `smoltcp` only). Runtime wires TUN TCP/UDP into existing rules/outbound, DNS hijack, owned auto-route and stop/failure cleanup. Privileged Linux netns HTTP/DNS/UDP echo remains fail-closed (`PHASE8A_NATIVE=1`); Parity only after that fixture. 8B macOS arm64 / 8C Windows follow; 8F before 8D/8E |
