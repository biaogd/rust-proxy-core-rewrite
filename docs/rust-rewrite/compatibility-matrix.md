...
| Named listeners | Oracle | Partial | Phase 6C-N proves named `shadowsocks` listeners with simple-obfs, shadow-tls v3 (fallback relay, concurrent accept, `handshake.proxy` leaf/group, empty-proxy `IN-TYPE,INNER`, fallback shutdown/reload bounds, config v1/v2 rejection), Rust-only shadow-tls `IN-USER`, 2022/EIH, UoT, password reload and validation corpus; remaining listener types stay open |
