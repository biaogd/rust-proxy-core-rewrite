...
| Linux amd64 — Phase 8F TUN network-change | Oracle | **Partial** (monitor implemented; native pass unclaimed until CI) | Default-route poll excluding TUN, snapshot re-protect (never via TUN), refuse-foreign exact prefixes, socket-level DIRECT/TUIC/Hysteria2 bind (no DIRECT host route; Windows TCP local port 0), QUIC clients retired on NIC change, `route-exclude-address` physical exceptions, resolver `reset_connections()` without fake-IP flush, 4096 TCP/UDP caps, bounded UDP reply queue, 1024 host-route cap fails closed. Privileged dual-uplink `PHASE8F_NATIVE=1` / `phase8f-linux-tun` is fail-closed and covers flap with the old uplink down, public DIRECT, same-IP REJECT, domain TUIC, and foreign-route conflict. Native Darwin/Windows NIC flap, UDP fragment/loss and TUN TCP half-close/RST are not in this gate. Not Parity |
