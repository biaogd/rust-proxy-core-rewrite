...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M2 covers top-level plus provider `ss` parsing with eight shared SIP004 AEAD, eight legacy stream ciphers, four 2022 TCP methods, AES-2022 single-hop EIH, native pre-2022 IPv4/IPv6 UDP, UoT v1/v2, simple-obfs HTTP/TLS TCP and controller views. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Shadowsocks (`ss`) | Oracle | **Parity in Phase 6C-M2 declared scope** | A–H proves eight shared SIP004 AEAD plus eight legacy stream methods over TCP/native UDP, providers/health and UoT v1/v2. I/J proves three standard 2022 methods and AES single-hop EIH over TCP; K adds ChaCha8-2022 TCP. L proves shared native UDP relay to explicit IPv6 destinations. M1/M2 adds embedded simple-obfs HTTP/TLS TCP with exact Host/SNI and lifecycle evidence. 2022 UDP is disabled after the pinned Go client panicked on an official Rust authority response. Other plugins, multi-hop EIH, Go-only methods, server direction and advanced socket/dialer options remain open |
