...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers top-level plus provider `ss` parsing with eight shared SIP004 AEAD, eight legacy stream ciphers, four 2022 TCP methods, AES-2022 single-hop EIH, native pre-2022 IPv4/IPv6 UDP, UoT v1/v2, simple-obfs HTTP/TLS, the documented v2ray-plugin TCP option surface and native `plugin: shadow-tls` v3. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Named listeners | Oracle | Partial | Phase 6C-N proves named `shadowsocks` listeners with simple-obfs, shadow-tls v3 (fallback relay, concurrent accept, proxy-observed `handshake.proxy` leaf/group, discriminating empty-proxy `IN-TYPE,INNER`, identity-changing fallback reload, config v1/v2 rejection), Rust-only shadow-tls `IN-USER`, 2022/EIH, UoT, password reload and fail-closed rejection of unimplemented listener/security fields; remaining listener types stay open |
...
| Shadowsocks | Yes | Yes | Oracle | Partial | Phase 6C-N proves ss-config named inbound TCP (AEAD, legacy, 2022/EIH, simple-obfs, shadow-tls v3 with fallback/concurrent accept/`handshake.proxy`/`IN-TYPE,INNER`/shutdown-reload bounds; Rust-only shadow-tls `IN-USER`) and UDP paths via `phase6c_shadowsocks_inbound.py`; full upstream interop matrix remains open |
...
| Shadowsocks (`ss`) | Oracle | **Partial — Phase 6C-M6 wire parity in declared TCP scope** | A–H proves eight shared SIP004 AEAD plus eight legacy stream methods over TCP/native UDP, providers/health and UoT v1/v2. I/J proves three standard 2022 methods and AES single-hop EIH over TCP; K adds ChaCha8-2022 TCP. L proves shared native UDP relay to explicit IPv6 destinations. M1/M2 adds simple-obfs HTTP/TLS; M5 closes documented v2ray-plugin TCP options including mux, early data, raw/fast upgrade, advanced TLS, mTLS and ECH; M6 adds native `plugin: shadow-tls` v1/v2/v3 over SS2022 TCP with hand-rolled ClientHello session-id HMAC, TLS 1.2/1.3 camouflage and post-handshake HMAC/XOR framing. **M6 Chrome `client-fingerprint` is partial only:** Rust advertises 10 cipher suites vs Go/uTLS `HelloChrome_133` 16 (RSA/CBC suites omitted because rustls aws-lc cannot negotiate them); non-`chrome` fingerprint labels are rejected at YAML load; `phase6c_shadowtls_clienthello_regression.py` per-runtime CI regression captures on-wire ClientHello through production ShadowTLS v3 paths (`NewShadowTLS` / `connect_with_session_id_generator`) and pins each runtime's documented cipher/extension baseline plus session-id HMAC — **not** Go/Rust wire parity evidence; BoringGREASEECH enc key material is a random 32-byte stand-in. Protocol wire parity remains in `phase6c_shadowsocks_shadow_tls.py`. Rust safely splits pathological mux writes above 65,535 bytes instead of copying Go's corrupt frame. 2022 UDP, multi-hop EIH, Go-only methods, server direction and advanced socket/dialer options remain open |
...
| Linux amd64 — Phase 6C-N Shadowsocks ss-config inbound | Oracle | Pending | `local-runtime` shard runs `phase6c_shadowsocks_inbound.py` including shadow-tls v3 fallback concurrency, `handshake.proxy` leaf/group, empty-proxy `IN-TYPE,INNER`, fallback shutdown/reload bounds, and Rust-only shadow-tls `IN-USER`; no native result is claimed before completion |
