...
| SSH | Oracle | **Partial — 6J-A TCP outbound** | YAML `type: ssh` password and/or private-key, optional `host-key`, reused `direct-tcpip` mux, mixed/SOCKS TCP, rules/groups/providers/health/reload in `phase6j_ssh_tcp.py` plus `protocol-ssh` `tcp_relay`. `udp: true` / `dialer-proxy` / TFO/MPTCP rejected. No inbound. Not Parity; three-platform CI pending |
