...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–D adds top-level and file-provider/selector VMess records with fail-closed validation for every native-TCP security mode and positive legacy AlterID. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VMess | Oracle | **Parity in Phase 6D-A–D native-TCP client scope** | `auto`, AES-128-GCM, ChaCha20-Poly1305, `none`/`zero` and AES-128-CFB pass independent-authority differentials with both AEAD (`alterId: 0`) and positive legacy AlterID headers. Evidence includes all framing combinations, positive counts 1/2/64, domain/IPv4/IPv6, small/128 KiB relay, half-close, failure lifecycle, controller shape and file-provider selector routing. Negative AlterID, UDP/XUDP, TLS, transports, mux and server direction remain open |
