...
| Proxy groups/select/fallback/url-test/load-balance | Oracle | Partial | Select/fallback/URL-test retain prior control and health behavior. All three load-balance strategies have REST/control and TCP wire evidence; automatic HTTP health adds startup, eager/lazy interval, timeout, touch activation, configured dial-failure thresholds, refused-connection activation and failover. Exact failure-window boundaries, delayed-handshake success reset, SOCKS5 and UDP remain |
