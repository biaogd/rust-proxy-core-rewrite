...
| Windows amd64 — Phase 8C TUN | Oracle | **Partial** (data path implemented; native pass unclaimed until CI) | Wintun load (`MIHOMO_WINTUN` / next-to-exe, official 0.14.1 zip hashed in `phase8c_tun.py`), `delete_driver(false)`, this-adapter metric 1, Linux-style split auto-route, TUN-adapter-only DNS, existing-adapter refusal. Privileged `PHASE8C_NATIVE=1` / `phase8c-windows-tun` is fail-closed. Not Parity |
