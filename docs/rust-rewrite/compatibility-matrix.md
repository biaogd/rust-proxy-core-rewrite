...
| AnyTLS | Yes | Protocol-dependent | Oracle | Partial | Phase 6G-A/B/C/D outbound native TLS TCP auth/padding/relay + session reuse/mux + UDP/UoT v2 + idle/heartbeat/recovery; inbound remains open |
...
| AnyTLS | Oracle | Partial — Phase 6G-A/B/C/D native TLS TCP + session reuse + UDP/UoT + idle/heartbeat/recovery | Config parse, password auth, padding, TCP/mux/UDP/idle differentials; Restls/ShadowTLS/JLS carriers remain open |
