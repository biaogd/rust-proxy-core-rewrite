...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-I covers top-level plus provider `ss` parsing with eight shared SIP004 AEAD, eight legacy stream ciphers, three standard 2022 TCP methods, native pre-2022 UDP, UoT v1/v2 and controller views. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Shadowsocks (`ss`) | Oracle | **Parity in Phase 6C-I declared scope** | A–H proves eight shared SIP004 AEAD plus eight legacy stream methods over TCP/native UDP, providers/health and UoT v1/v2. Phase I proves three standard single-PSK 2022 methods over TCP with exact key validation. 2022 UDP is disabled after the pinned Go client panicked on an official Rust authority response. Go-only methods, EIH, plugins, server direction, IPv6 UDP, UoT connect mode and advanced socket/dialer options remain open |
