...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–I adds top-level and file-provider/selector VMess records with fail-closed validation for every security mode, positive legacy AlterID, standard/packet-address/XUDP UDP selection, TLS/WS/WSS variants, HTTP/1/H2 and single-stream gRPC/Gun TCP transports. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VMess | Oracle | **Parity in Phase 6D-A–I declared client scope** | Every supported security mode passes independent-authority differentials with AEAD and positive legacy AlterID headers. Phase 6D-E proves ordinary/packet-address/XUDP UDP; F/G prove TLS, WS/WSS early data and raw Upgrade; H proves HTTP/HTTPS first-write obfuscation plus h2c/H2 PUT streams; I proves default/named/custom-path gRPC/Gun envelopes over one h2c/H2 stream. Negative AlterID, oversized UDP edges, packet-address FQDN payloads, UDP over non-native transports, gRPC pooling, advanced TLS, remaining outer transports, general TCP mux and server direction remain open |
