...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–F adds top-level and file-provider/selector VMess records with fail-closed validation for every security mode, positive legacy AlterID, standard/packet-address/XUDP UDP selection and native TLS/WS/WSS TCP transports. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VMess | Oracle | **Parity in Phase 6D-A–F declared client scope** | Every supported security mode passes independent-authority differentials with AEAD and positive legacy AlterID headers. Phase 6D-E proves ordinary fixed-destination UDP, packet-address IPv4/IPv6 and XUDP multi-destination framing. Phase 6D-F proves native TLS, plaintext WS and WSS TCP with exact SNI/Host/path, custom roots, name override, skip verification, untrusted rejection, large relay and half-close. Negative AlterID, oversized UDP edges, packet-address FQDN payloads, UDP over WS, early data, advanced TLS, remaining outer transports, general TCP mux and server direction remain open |
...
| Shared outbound transport/security variants | Oracle | Partial | Phase 6D-F reuses the existing Rustls client and Tungstenite WebSocket boundaries for VMess TLS/WS/WSS TCP, with independent SNI/Host/path and trust evidence. HTTP/2, gRPC/Gun, xHTTP/H3, mKCP, Mekya, early/raw-upgrade variants, Reality, ECH, JLS, ReSTLS, ShadowTLS and TLSMirror remain per-consumer gates |
