...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A adds top-level and file-provider/selector VMess records with fail-closed native-TCP validation. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VMess | Oracle | **Parity in Phase 6D-A native-TCP scope** | AEAD `auto`, AlterID 0, domain/IPv4 destinations, small/128 KiB relay, half-close, failure/credential lifecycle, controller shape and file-provider selector routing pass `phase6d_vmess_tcp.py`; UDP/XUDP, other ciphers/AlterID, TLS, transports, mux and server direction remain open |
