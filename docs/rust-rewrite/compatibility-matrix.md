...
| `/proxies`, `/group`, delay and selection | Oracle | Partial | `phase5d_proxies.py` proves all seven built-ins and GLOBAL; `phase5c_selector.py` adds exact configured HTTP/selector detail and live selection-driven routing; selector fixtures isolate HOME/XDG/Mihomo profile state so persisted choices cannot cross scenarios; local HEAD delay, expected/timeout validation, history/extra health and current TCP/UDP mode effects pass; other groups, provider members, HTTPS and exhaustive failure/reload/persistence remain unclaimed |
