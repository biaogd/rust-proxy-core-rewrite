...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A/B adds top-level and file-provider/selector VMess records with fail-closed native-TCP validation for the accepted AEAD options. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VMess | Oracle | **Parity in Phase 6D-A/B native-TCP scope** | `auto`, explicit AES-128-GCM and ChaCha20-Poly1305 with independent/combined global padding and authenticated length pass `phase6d_vmess_tcp.py` and `phase6d_vmess_aead.py`; evidence includes AlterID 0, domain/IPv4/IPv6 destinations, small/128 KiB relay, half-close, failure lifecycle, controller shape and file-provider selector routing. UDP/XUDP, `none`/`zero`/legacy ciphers and AlterID, TLS, transports, mux and server direction remain open |
