...
| dialer-proxy TCP chains | Oracle | **Partial — 7T1-A** | Shared TCP dial entry; B with `dialer-proxy: A` reaches B's server via A. First slice HTTP↔SOCKS5 with path-proving fixtures (`phase7t1_dialer_proxy_tcp.py`). Existing TCP protocols wire through the same dial. Missing/self/cycle refs fail closed. SSH/Hy2/TUIC/WG `dialer-proxy`, UDP chains and sing-mux remain rejected. See `OUT-21-dialer-proxy-chains.md` |
