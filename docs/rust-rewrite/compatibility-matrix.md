...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-D covers top-level plus inline/file-provider `ss` with the three standard SIP004 AEAD ciphers, optional UDP and controller views. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Proxy providers and health checks | Oracle | Partial | Phase 5C completes inline/file/HTTP/HTTPS vehicles, HTTP/SOCKS5 parsing, filters/name transforms, real provider health, manual/interval/file-watch refresh, digest-bound ETag restart, caches, SIGHUP and concurrent rollback. Phase 6C-D adds Shadowsocks parsing, controller membership and TCP/UDP consumption for inline/file vehicles. Named download proxies, general override expressions, encrypted payloads, Shadowsocks HTTP-provider lifecycle/health and later adapter types remain unclaimed |
...
| Shadowsocks (`ss`) | Oracle | **Parity in Phase 6C-D native SIP004 AEAD scope** | The Phase 6C A–C differentials prove all three accepted ciphers over domain/IPv4 TCP and UDP, TCP framing/half-close, mixed/SOCKS5 UDP association reuse and controller capability. `phase6c_shadowsocks_provider.py` additionally proves selector switching and real TCP/UDP forwarding for inline/file-provider SS members. Stream/extra/2022 ciphers, UoT, plugins, HTTP-provider lifecycle/health, server direction, IPv6 UDP and advanced socket/dialer options remain open |
