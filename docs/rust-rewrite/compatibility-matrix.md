...
in progress (8A Linux + 8B Darwin arm64 + 8C Windows x86_64)**; see [roadmap](roadmap.md#next-work-priority--2026-09-09).
...
| Cross-platform and future-resource hook ordering | Oracle | Partial | Windows uses `cmd.exe /C` and the native differential drives a real `CTRL_BREAK_EVENT`; readiness now covers loaded native runners and failure cleanup releases redirected logs before temporary-directory removal. Native Windows revalidation and every future runtime resource joining the readiness/shutdown barrier remain pending |
...
| LAN allow/deny, skip-auth and fixed-listener socket policy | Oracle | **Parity in declared fixed-listener scope** | `phase5f_lan_policy.py` isolates IPv4/IPv6 bind/filter/auth/rebind from outbound policy and proves TFO, keepalive and concurrent-domain settings without conflating them with MPTCP; native MPTCP behavior and privileged global-unicast mark evidence remain pending |
...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–L adds the declared VMess surface. Phase 6E-A–N adds VLESS native/TLS/WS/HTTP/H2/gRPC, UDP/XUDP over common carriers, Vision, REALITY, pooled Gun, common HTTP/2 xHTTP modes and basic XMUX. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| TUN and route settings | Oracle | **Partial** (8A Linux + 8B Darwin + 8C Windows + 8F monitor apply; native Parity unclaimed) | Common keys parse; unimplemented `strict-route` / `endpoint-independent-nat` / `udp-timeout` / `disable-icmp-forwarding` reject non-default values; `stack: smoltcp` accepted; Go `system`/`gvisor`/`mixed` rejected without remap. Darwin `tun.device` must be empty or `utunN` (no remap of `tun0`/`utun`). Windows refuses to take over an existing named adapter and does not call `WintunDeleteDriver`. Unprivileged evidence: config unit tests and `compat/scripts/phase8a_tun.py` / `phase8b_tun.py` / `phase8c_tun.py` / `phase8f_tun.py`. Privileged Linux netns fixture covers manual then auto-route, DNS hijack/fake-IP HTTP, large transfer, UDP echo, init-failure rollback, failed-reload restore of the previous generation (old mixed-port kept, new port closed; same-port replacement then TUN failure must restore localhost and release the wildcard; stop-old then unassigned bind failure must restore the old port without stopping TUN), and stop cleanup (`PHASE8A_NATIVE=1`, fail-closed). Privileged Darwin arm64 fixture covers utun, scutil DNS merge/restore, TUN-DNS /32 on utun (extras such as SupplementalMatchDomains are snapshotted and not wiped with `d.init`), split auto-route, HTTP/UDP through TUN and invalid-device leftover (`PHASE8B_NATIVE=1`, fail-closed; native pass unclaimed). Privileged Windows x86_64 fixture covers Wintun load (official 0.14.1 zip, pinned SHA-256; Rust uses the scratch DLL via `MIHOMO_WINTUN`, Go gets a next-to-exe copy, and WinError 32 reuses a matching digest only, otherwise isolates Go+DLL), split default auto-route, TUN-adapter-only DNS, HTTP/UDP through TUN, missing-DLL/existing-adapter leftovers (`PHASE8C_NATIVE=1`, fail-closed). 8F polls the physical default excluding TUN (IPv4 and IPv6 separately), re-protects captured-family hosts via that snapshot (never TUN; uncaptured IPv6 is skipped rather than routed via an IPv4 gateway), refuses foreign exact-prefix overwrites, binds DIRECT/TUIC/Hysteria2 sockets to the physical interface (no DIRECT host route; Windows TCP local port 0), retires QUIC clients and rebinds DIRECT UDP on NIC change, matching-family auto-detect bind (pure IPv6 is not lost; captured family without physical egress is refused), skips NIC bind for loopback UDP remotes, installs `route-exclude-address` physical exceptions, Darwin-reapplies scutil DNS, and resets resolver connections without flushing fake-IP; the dual-uplink netns flap takes the old uplink down and covers public DIRECT, same-IP REJECT, domain TUIC, same-socket DIRECT UDP, mixed/SOCKS UDP to localhost, Hysteria2 socket bind, and foreign-route conflict cases (`PHASE8F_NATIVE=1`). No Parity until those jobs report success |
...
| Profile persistence | Oracle | Partial | Darwin fake-IP state plus selector and fixed-fallback choices persist in the shared Go-compatible bbolt `cache.db`; bidirectional Go/Rust group interchange and `store-selected: false` pass. The pinned `biaogd/bbolt-rs` backend now compiles and tests upstream on Windows and the Rust product uses the same persistence path on every platform, but native Windows product restart/interchange evidence remains pending; corruption recovery and other profile state also remain unclaimed |
...
| Full mixed HTTP/SOCKS listener | Yes | SOCKS UDP | Oracle | Partial | Phase 3 declared HTTP/SOCKS4/4a/5 TCP and local SOCKS5 UDP subset; Phase 5F completes the declared local IPv4/IPv6 client-keyed NAT, multiple write-backs, destination fan-out, control-close/generation retention and bounded-pressure scope. Exact 60-second expiry is configured in CI; broader HTTP and remote UDP/UoT remain |
...
| TUN | Yes | Yes | Oracle | **Partial** (Linux 8A + Darwin 8B + Windows 8C data path + 8F monitor; native pass unclaimed until CI) | Parse + stack identity: `compat/scripts/phase8a_tun.py` / `phase8b_tun.py` / `phase8c_tun.py` / `phase8f_tun.py` and config unit tests (`tun-rs` 2.8.9 + `netstack-smoltcp` 0.2.4; `smoltcp` only). Runtime wires TUN TCP/UDP into existing rules/outbound, DNS hijack, owned auto-route and stop/failure cleanup on Linux, Darwin arm64 and Windows x86_64. Darwin adds utun, split default prefixes, and scutil DNS merge/restore (extra keys snapshotted; apply does not `d.init`). Windows loads Wintun without vendoring `wintun.dll`, never deletes the shared driver, sets metric 1 on this adapter only, uses Linux-style `0.0.0.0/1`+`128.0.0.0/1` auto-route, and sets DNS on the Wintun adapter only. Privileged Linux netns HTTP/DNS/UDP echo, fake-IP reverse mapping, Go `system` vs Rust `smoltcp` body match, init-failure leftover check, failed-reload restore of the previous generation (old mixed-port kept, new port closed; same-port replacement then TUN failure restores localhost and releases the wildcard; bind-fail after stop restores the old port), and stop cleanup are implemented and fail-closed (`PHASE8A_NATIVE=1`). Privileged Darwin arm64 HTTP/UDP/system-DNS/auto-route/cleanup is fail-closed (`PHASE8B_NATIVE=1`). Privileged Windows x86_64 Wintun HTTP/UDP/adapter-DNS/auto-route/missing-DLL/existing-adapter cleanup is fail-closed (`PHASE8C_NATIVE=1`). 8F adds default-interface polling (TUN excluded; IPv4 and IPv6 separately), snapshot re-protect for captured families (never via TUN; uncaptured IPv6 skipped), refuse-foreign exact prefixes, socket-level DIRECT/TUIC/Hysteria2 bind (no DIRECT host route; Windows TCP local port 0), QUIC clients retired and DIRECT UDP rebound on NIC change, matching-family auto-detect bind (pure IPv6 is not lost; captured family without physical egress is refused), loopback UDP remotes skipping NIC bind, `route-exclude-address` physical exceptions, Darwin DNS re-apply, resolver reset without fake-IP flush, 4096 TCP/UDP caps, and a bounded UDP reply queue; privileged dual-uplink flap takes the old uplink down and covers public DIRECT, same-IP REJECT, domain TUIC, same-socket DIRECT UDP, mixed/SOCKS UDP to localhost, and foreign-route conflict (`PHASE8F_NATIVE=1` / `phase8f-linux-tun`). UDP fragment/loss, TUN TCP half-close/RST, Android netlink and Darwin/Windows FFI monitors remain out of this gate. Parity only after those jobs report success. 8D/8E remain later |
| Shadowsocks | Yes | Yes | Oracle | Partial | Phase 6C-N implements the roadmap's declared ss-config/named TCP and UDP first-server slice. The corrected full Go/Rust `phase6c_shadowsocks_inbound.py` passes on Darwin arm64 after the protocol-crate refactor. **IN-B** adds standard SS2022 UDP inbound (three methods) with product-path replay via `phase_inb_shadowsocks_2022_udp.py`; ChaCha8 UDP stays rejected. Full inbound cipher matrix and remaining server features stay open |
| Snell | Yes | Yes | Oracle | Partial — 7E-A/B/C outbound TCP + v3 UDP + simple-obfs | Versions 1–3 TCP client, v3 UDP ASSOCIATE, HTTP/TLS simple-obfs; inbound/pool/remaining obfs remain rejected |
| VMess | Yes | Transport-dependent | Oracle | **Partial — IN-E TLS + WSS + gRPC + XUDP + REALITY** | Named `type: vmess` TLS, `ws-path` WSS, `grpc-service-name` Gun TCP, Mux/XUDP, and `reality-config` REALITY auth via `phase_ine_vmess_tls.py` / `phase_ine_vmess_websocket.py` / `phase_ine_vmess_grpc.py` / `phase_ine_vmess_xudp.py` / `phase_ine_vmess_reality.py`; AEAD alterId 0 only; combined ws+grpc / mKCP/Mekya / plain TCP rejected; REALITY dest fallback deferred; outbound remains Phase 6D (+ TCP REALITY client for inbound evidence) |
| VLESS | Yes | Transport-dependent | Oracle | Partial | Phase 6E-A–N client scopes; inbound/server direction remains open |
| Trojan | Yes | Yes | Oracle | **Partial — IN-C TLS + WSS + gRPC + REALITY + IN-H-A lifecycle** | Named `type: trojan` TLS, `ws-path` WSS, `grpc-service-name` Gun TCP+UDP-over-TLS, and `reality-config` REALITY auth via `phase_inc_trojan_tls.py` / `phase_inc_trojan_websocket.py` / `phase_inc_trojan_grpc.py` / `phase_inc_trojan_reality.py`; **IN-H-A** adds invalid-reload rollback, listener removal, cert path rotation, credential isolation, Rust slow-handshake bound (`phase_inh_trojan_lifecycle.py`); combined ws+grpc / `ss-option` rejected; REALITY dest fallback deferred; outbound remains Phase 6F |
...
| AnyTLS | Yes | Protocol-dependent | Oracle | **Partial — IN-G TLS TCP inbound** | Phase 6G-A/B/C/D/E outbound native TLS TCP auth/padding/relay + session reuse/mux + UDP/UoT v2 + idle/heartbeat/recovery + ShadowTLS/JLS carriers (Restls dial blocked on shared Restls client transport); **IN-G** adds named `type: anytls` TLS TCP inbound (`phase_ing_anytls_tcp.py`); UoT inbound and camouflage carriers remain open |
...
| Full MATCH behavior and routing modes | Oracle | Partial | `phase5d_modes.py` proves live Rule/Direct/Global switching; `phase6a_builtins.py` adds default GLOBAL configured-HTTP selection plus custom GLOBAL replacement on mixed TCP. Future inbound families and remote UDP remain unclaimed |
...
| Process name/path variants and UID | Oracle | **Partial — W2.1:** Linux PROCESS-NAME/PATH exact + UID | Per-OS process fixtures; regex/wildcard open |
| Phase 2 rematch-name pure metadata matcher | Oracle | **Parity** | Rematch mutation followed by `REMATCH-NAME` observation |
| Inbound type/user/name matchers | Oracle | **Partial** | Phases 5B3a–5B3c plus 5B UDP prove fixed HTTP/SOCKS/mixed TCP+UDP metadata; Phase 6C-N adds `IN-TYPE,INNER` for shadow-tls handshake dial routing and Rust-only shadow-tls `IN-USER` username metadata (Go sing-shadowsocks inbound does not attach shadow-tls user yet); both default UDP sockets report SOCKS5/`DEFAULT-SOCKS` and intentionally carry no TCP auth user; other named protocols remain pending |
...
| Proxy groups/select/fallback/url-test/load-balance | Oracle | Partial | Select/fallback/URL-test retain prior control and health behavior. PR 19 review corrects successful 0ms probes being misclassified as timeouts; fixed-clock group regression added, execution pending. All three load-balance strategies have REST/control and TCP wire evidence; automatic HTTP health adds startup, eager/lazy interval, timeout, touch activation, configured dial-failure thresholds, refused-connection activation and failover. Authenticated SOCKS5 fallback members additionally pass startup/manual/eager health, refused-connection activation and survivor routing. Exact failure-window boundaries, delayed-handshake success reset, SOCKS5 URL-test/load-balance/error coverage and UDP remain |
...
| Full DIRECT TCP/UDP/platform options | Oracle | Partial | Phase 1/3 local TCP plus SOCKS UDP; Phase 5F applies interface, Linux/Android routing mark, keepalive and concurrent-domain policy to DIRECT/current HTTP/SOCKS5 dials, plus TFO/MPTCP on fixed inbound TCP. Phase 6A2 adds configured DIRECT TCP/UDP and provider/group selection. Privileged nonzero-mark, rich per-adapter option combinations, CI timeout result and remote UDP adapters remain unclaimed |
| REJECT / REJECT-DROP | Oracle | **Parity in declared fast TCP/UDP scope** | `phase6a_builtins.py` proves immediate REJECT and held REJECT-DROP TCP; `phase6a_simple_adapters.py` proves configured REJECT plus reserved REJECT/REJECT-DROP silent UDP drop. Linux CI additionally enables the oracle's exact 60-second TCP expiry gate, which remains unclaimed until completion |
...
| HTTP | Oracle | **Parity in native adapter scope** | Phase 6B proves plaintext/TLS CONNECT, auth/partial credentials, default/custom headers, exact status and malformed/delayed framing, SNI/name override, custom roots, skip, SHA-256 pinning, client certificates, controller/group/provider use and the adapter's no-UDP/no-UoT boundary. Cross-cutting TCP `dialer-proxy` composition is Phase 7T1-A / OUT-21 |
| SOCKS5 | Oracle | **Parity in native adapter scope** | Phase 6B proves plaintext/TLS CONNECT, no-auth/auth/partial/overlength credential wire behavior, domain/IPv4/IPv6 addressing, method/reply/retry lifecycle, client certificates/pinning and authenticated plaintext/TLS UDP ASSOCIATE reuse with `udp: true`, `uot: false`. Cross-cutting TCP `dialer-proxy` composition is Phase 7T1-A / OUT-21 |
| dialer-proxy TCP chains | Oracle | **Partial — 7T1-A** | Shared TCP dial entry; B with `dialer-proxy: A` reaches B's server via A. HTTP↔SOCKS5 path-proving differentials pass (`phase7t1_dialer_proxy_tcp.py`); SS/SSR/Snell also dial through the shared entry (unchained Snell v2 keeps its pool). `dialer-proxy` + UDP fails closed at load. Delay/health uses the shared dial entry. VMess/VLESS/Trojan/AnyTLS may parse `dialer-proxy` but are runtime-rejected until wired. Missing/self/cycle refs fail closed. SSH/Hy2/TUIC/WG `dialer-proxy`, UDP chains and sing-mux remain rejected. See `OUT-21-dialer-proxy-chains.md` |
| Shadowsocks (`ss`) | Oracle | **Partial — Phase 6C-M6 client wire parity, 6C-N first-server parity and 6C-O standard 2022 UDP outbound in their declared scopes** | A–H proves eight shared SIP004 AEAD plus eight legacy stream methods over TCP/native UDP, providers/health and UoT v1/v2. I/J proves three standard 2022 methods and AES single-hop EIH over TCP; K adds ChaCha8-2022 TCP. L proves shared native UDP relay to explicit IPv6 destinations. M1/M2 adds simple-obfs HTTP/TLS; M5 closes documented v2ray-plugin TCP options; M6 adds native `plugin: shadow-tls` v1/v2/v3 over SS2022 TCP. M6 Chrome advertises all 16 Go/uTLS suites, but its wrapper-specific extension regression remains per-runtime rather than wire parity. Phase 6C-N separately validates the first inbound/server slice. Phase 6C-O enables native UDP outbound for the three standard 2022 methods plus AES single-hop EIH. ChaCha8 UDP, 2022 UoT, multi-hop EIH, Go-only methods, advanced socket/dialer options and inbound features outside 6C-N remain open |
| ShadowsocksR (`ssr`) | Oracle | Outbound A–D (gaps remain) | TCP origin/auth_aes128_*/auth_sha1_v4/auth_chain_a/b; plain/http_*/tls1.2_ticket_* camouflage/random_head; stream ciphers + none/dummy; UDP outbound skips TCP obfs (Go parity); D stress/negatives/soak. Per-adapter authentication identity correction: `phase7d_ssr_identity.py` passed locally on macOS arm64, 2026-09-09; six profiles each opened/retained 96/96 connections in Go and fixed Rust, unchanged server client limit (pre-fix MD5 plain/TLS retained only 64/96). Native CI revalidation pending. No inbound SSR; mudb multi-user e2e unclaimed |
| VMess | Oracle | **Parity in Phase 6D-A–L declared client scope (+ UDP carriers + TCP REALITY)** | A–J retain the security, AlterID, UDP, TLS/WS/HTTP/H2 and pooled Gun evidence. E2 adds ordinary/packet-address/XUDP over native TLS, plaintext WS, WSS and Gun/gRPC by reusing those carriers, with 5s setup timeout/shutdown cancel. K adds VMess TCP over V2Ray mKCP with default/simple authentication, AES-GCM seed, all camouflage headers, Go-contract fast-ACK/RTO/congestion state, deterministic 14%/25% semantic DATA loss, ACK loss, duplication/reordering and the oracle's full-close-on-half-close behavior. L adds TLS Mekya over negotiated H2 and HTTP/1.1 with the same close lifecycle. **W3.3** adds native TCP REALITY client (`reality-opts` + Chrome fingerprint) for named inbound evidence (`phase_ine_vmess_reality.py`). UDP over HTTP/H2/mKCP/Mekya, randomized long-duration impairment, advanced TLS, xHTTP/H3, general TCP mux and remaining server carriers remain open |
...
| Snell | Oracle | **Partial — 7E-A/B/C/D versions 1–3 TCP + v3 UDP + simple-obfs + v2 reuse** | YAML `type: snell` `psk` + version 1/2/3 through mixed/SOCKS TCP in `phase7e_snell_tcp.py`; v3 `udp: true` through mixed/SOCKS UDP ASSOCIATE in `phase7e_snell_udp.py`; `obfs-opts` HTTP/TLS in `phase7e_snell_obfs.py`; v2 ConnectV2 pool in `phase7e_snell_reuse.py`. v1 ChaCha20-Poly1305, v2/v3 AES-128-GCM, Argon2id KDF. Remaining `obfs-opts`/v4/v5/`dialer-proxy` and inbound remain rejected. Not Parity; three-platform CI is the new snell shard |
| Trojan | Oracle | **Partial — Phase 6F-A–E client + IN-C TLS/WSS/gRPC/REALITY server** | A proves native TLS TCP authentication/framing/relay; B proves native TLS UDP framing, splitting and reuse; C proves TCP and UDP over WSS with path, Host/custom headers and WebSocket ALPN; D proves TCP and UDP over HTTP/2 Gun/gRPC, service path, User-Agent and pooled client controls; E proves native TCP and UDP over REALITY with Chrome fingerprint, public-key/short-id authentication and strict unsupported-combination rejection. **IN-C** adds named Trojan TLS, `ws-path` WSS, `grpc-service-name` Gun inbound TCP+UDP-over-TLS, and `reality-config` REALITY auth Accept (`phase_inc_trojan_tls.py`, `phase_inc_trojan_websocket.py`, `phase_inc_trojan_grpc.py`, `phase_inc_trojan_reality.py`); shared WS+gRPC mux / `ss-option` / dest fallback remain open. TLS name override/skip still rely on shared transport evidence |
...
| WireGuard / AmneziaWG | Oracle | **Partial — 6I-A/B/C single-peer TCP+UDP+lifecycle outbound** | YAML `type: wireguard` userspace Noise (`defguard_boringtun`) + smoltcp TCP/UDP stack, mixed/SOCKS TCP+UDP, inner IPv6, `remote-dns-resolve`, keepalive, `refresh-server-ip-interval`, rehandshake, TUN peer-IP loop-avoidance, rules/groups/providers/health/reload in `phase6i_wireguard_tcp.py` / `phase6i_wireguard_udp.py` / `phase6i_wireguard_lifecycle.py`. No OS TUN / admin for mixed. AmneziaWG, `peers`, `ip-stack` and inbound remain rejected/deferred. Not Parity; three-platform CI pending |
| SSH | Oracle | **Partial — 6J-A/B TCP+lifecycle outbound** | YAML `type: ssh` password and/or private-key, optional `host-key`, applied `host-key-algorithms`, transport TCP keepalive, reused `direct-tcpip` mux, authority-restart reconnect, encrypted key-file passphrase, mixed/SOCKS TCP, rules/groups/providers/health/reload/soak in `phase6j_ssh_tcp.py` / `phase6j_ssh_lifecycle.py` plus `protocol-ssh` `tcp_relay`/`lifecycle`. PR 19 review adds fail-closed host-key/config validation, RSA SHA-2 negotiation, independent channel confirmation, and cancellation/zero-ms health regressions; new execution evidence pending. `udp: true` / `dialer-proxy` / TFO/MPTCP rejected. No inbound. SSH-protocol keepalive and exact deadline identity not claimed (Rust setup/channel timeout: 10s). Not Parity; three-platform CI pending |
| Mieru | Oracle | Not started | Client/mux interop |
| AnyTLS | Oracle | **Partial — Phase 6G-A/B/C/D/E client + IN-G TLS TCP server** | Config parse, password auth, padding, TCP/mux/UDP/idle/carrier differentials; Restls dial blocked on shared Restls TLS client transport. **IN-G** named inbound TLS TCP via `phase_ing_anytls_tcp.py`; UoT inbound and camouflage carriers remain open |
...
| Shared outbound transport/security variants | Oracle | Partial | Phase 6D-F–L covers TLS/WS/Upgrade/HTTP/H2, pooled Gun, mKCP and Mekya for VMess; 6D-E2 reuses those carriers for VMess XUDP. Phase 6E-I–M adds VLESS UDP carrier composition, REALITY+Vision, pooled Gun and common HTTP/2 xHTTP/basic XMUX. H3 and advanced xHTTP controls, randomized long-duration mKCP impairment, other consumers, ECH, JLS, ReSTLS and TLSMirror remain separate gates |
...
| Phase 4E11 DoT concurrency/timeout/reset/retry subset | Oracle | **Parity** | `DNS-06`; 12 concurrent misses/8 idle cap, five-second response timeout, validated config-generation pool reset, stale one-shot reconnect and fresh-failure retry bound in `compat/scripts/phase4e11.py` |
...
| Phase 4E18 DoQ reuse/concurrency/retry/reset subset | Oracle | **Parity** | `DNS-09`; sequential and eight overlapping streams on one connection, two bounded `NO_ERROR` reconnect attempts, same-config generation reset and full-handshake `DidResume=false`/`Used0RTT=false` observations in `compat/scripts/phase4e18.py` |
...
| Phase 4F7 resolver-set composition core | Oracle | **Partial** | All accepted transport forms compose in default/main/fallback/direct/proxy-server sets; deterministic multi-client selection, fallback filtering, direct-follow-policy and explicit default/proxy lookups pass in `compat/scripts/phase4f7.py`; runtime proxy TCP/UDP dials (SSR/SS/VMess/VLESS/Trojan/HTTP/SOCKS/Hysteria2) resolve through `dns.proxy-server-nameserver` with main-resolver fallback; UDP destination lookups prefer configured DNS; remaining bootstrap consumers stay pending |
...
| DNS hijack through TUN | Oracle | **Partial** (8A netns + 8B Darwin system DNS + 8C Windows adapter DNS) | `compat/scripts/phase8a_tun.py` with `PHASE8A_NATIVE=1` sends UDP/53 through the TUN device, requires a fake-IP in `198.19.0.0/16`, then reverse-maps DOMAIN rules for HTTP/UDP. Darwin 8B also rewrites the primary service DNS via `scutil` to the TUN next address and restores it on stop (`PHASE8B_NATIVE=1`). Windows 8C sets DNS on the Wintun adapter only via `netsh` (never Ethernet/Wi-Fi/other VPN NICs) and restores DHCP/static on that adapter (`PHASE8C_NATIVE=1`). Loop-avoidance covers literal DNS/proxy IPs only. No Parity until the privileged jobs report success |
...
| --- | --- | --- | --- |
| TCP/TLS/Unix/Windows pipe listeners | Oracle | **Parity in declared transport scope** | `phase5d_transports.py` proves simultaneous TCP/TLS/Unix, Unix auth bypass/mode, TLS trust, UI-path and address hot replacement on Darwin; native Windows named-pipe differential is configured, and privileged nonzero Linux routing-mark evidence remains pending |
...
| `/configs` GET/PUT/PATCH and `/configs/geo` | Oracle | Partial | Phase 5D proves executable config transactions and path rollback; Phase 5E adds exact `POST /configs/geo` behavior for current geodata consumers; Phase 5F adds live LAN, TFO/MPTCP, interface/mark, concurrent-dial and keepalive snapshots plus PATCH for the two Go-patchable dial fields. Unimplemented config runtimes and broader consumers remain feature gates |
...
| `/rules` and disable operation | Oracle | Partial | `phase5d_rules.py` proves ordered DomainSuffix/MATCH inventory fields, shared hit/miss counters and timestamps, disable/enable routing side effects, ignored indexes and malformed-body behavior; `phase5e_geo_rules.py` proves GeoIP/GeoSite/SrcGeoIP type, normalized payload and record-size behavior; exhaustive rule payload rendering and Geo reload-state behavior remain unclaimed |
...
| Proxy and rule provider APIs | Oracle | Partial | Phase 5C proves proxy/rule list/detail where declared, PUT, provider health, inline/file/HTTP/HTTPS vehicles, interval/file-watch refresh, durable ETag restart, concurrent transactional rollback and config-generation removal. Named download proxies, general override expressions, encrypted payloads and future adapters remain unclaimed |
...
| Restart and upgrade APIs | Oracle | Partial | Phase 5D proves Unix same-PID `exec` and Windows replacement-process restart, including controller recovery and child cleanup; Phase 5E proves `/upgrade/ui` success/error/rollback and `/upgrade/geo` success/error/rollback. Signed/versioned core `/upgrade` remains deliberately unimplemented |
...
| Darwin arm64 — Phase 5C aggregate | Oracle | **Parity in declared pre-Phase-6 adapter scope** | All 27 group/provider differentials passed consecutively, including HTTPS, durable ETag, rule providers, file watch, health and concurrent cleanup, 2026-08-27 |
...
| Darwin arm64 — Phase 5F1 LAN/socket policy | Oracle | **Parity in declared fixed-listener scope** | Native fixed HTTP/SOCKS/mixed IPv4/IPv6 wildcard/explicit/non-loopback bind, same-port PATCH rebind, allow/deny, skip-auth, TFO/MPTCP-enabled relay, interface/domain-concurrency path and controller snapshot differential passed, 2026-08-28; nonzero mark is not a Darwin claim |
| Darwin arm64 — Phase 5F2 UDP NAT | Oracle | **Parity in declared fast local scope** | Native IPv4/IPv6 stable outbound-port reuse, destination fan-out, sequential/burst write-back, control-close/generation retention and 256-packet bounded-pressure recovery passed, 2026-08-28; exact 60-second timeout is a Linux CI slow gate and remote adapters remain |
| Darwin arm64 — Phase 6A1 built-ins/GLOBAL | Oracle | **Parity in declared fast scope** | Native built-in/controller, PASS→COMPATIBLE, immediate/held rejection, default/configured/custom GLOBAL routing and delay differential passed, 2026-08-28; exact 60-second expiry remains a Linux CI gate |
...
| Darwin arm64 — Phase 6B3 HTTP/SOCKS5 completion | Oracle | **Parity in native adapter scope** | Native custom-root/name override/client-certificate/pinning, partial HTTP credentials, malformed/delayed response, overlength SOCKS auth and plaintext/TLS UDP ASSOCIATE differentials passed, 2026-08-28; OUT-21 chaining remains separate |
| Darwin arm64 — Phase 6C-A Shadowsocks AES-128-GCM TCP | Oracle | **Parity in declared scope** | Native official-library authority differential passed for domain-target encrypted TCP relay, MATCH rejection and controller view, 2026-08-29; other ciphers/UDP/UoT/plugins/server direction remain open |
| Darwin arm64 — Phase 6C-B Shadowsocks SIP004 AEAD TCP | Oracle | **Parity in declared scope** | Native three-cipher official-library authority differential passed for domain/IPv4, 128 KiB relay, half-close and process survival, 2026-08-29; UDP/UoT/plugins/2022/server direction remain open |
| Darwin arm64 — Phase 6C-C Shadowsocks SIP004 AEAD UDP | Oracle | **Parity in declared scope** | Native three-cipher mixed/SOCKS5 UDP differential passed for IPv4/domain relay, per-client session reuse, controller capability and process survival, 2026-08-29; IPv6 UDP/UoT/plugins/2022 remain open |
| Darwin arm64 — Phase 6C-D Shadowsocks local providers/groups | Oracle | **Parity in declared scope** | Native inline/file provider parsing, member/controller views, selector switching and SS TCP/UDP wire relay differential passed, 2026-08-29; HTTP-provider lifecycle and automatic health remain open |
| Darwin arm64 — Phase 6C-E Shadowsocks HTTP provider/health | Oracle | **Parity in declared scope** | Native initial HTTP download, manual record refresh, fresh-cache restart, selector/controller state, TCP/UDP relay and real SS TCP health-probe differential passed, 2026-08-29; scheduled/ETag stress remains open |
| Darwin arm64 — Phase 6C-F Shadowsocks UDP-over-TCP | Oracle | **Parity in declared scope** | Native config/version, controller capability and official-library encrypted TCP authority differential passed for sing UoT v1/v2 IPv4/domain-originated relay and session reuse, 2026-08-29; plugins/2022/connect mode remain open |
| Darwin arm64 — Phase 6C-G Shadowsocks legacy stream ciphers | Oracle | **Parity in declared scope** | Native eight-method official-library differential passed for config, domain/large/half-close TCP and IPv4/domain UDP, 2026-08-29; Go-only extra methods/plugins/2022 remain open |
| Darwin arm64 — Phase 6C-H Shadowsocks extra AEAD ciphers | Oracle | **Parity in declared scope** | Native five-method official-library differential passed for config, domain/large/half-close TCP and IPv4/domain UDP, 2026-08-29; remaining Go-only methods/plugins/2022 remain open |
| Darwin arm64 — Phase 6C-I Shadowsocks 2022 TCP | Oracle | **Parity in declared scope** | Native three-method single-PSK config and official-library TCP differential passed for domain/large/half-close relay, 2026-08-29; native UDP outbound is Phase 6C-O |
| Darwin arm64 — Phase 6C-J Shadowsocks 2022 EIH | Oracle | **Parity in declared scope** | Native AES-128/256 single-hop EIH config and official-library TCP differential passed for domain/large/half-close relay, 2026-08-29; multi-hop EIH remains open and native UDP outbound is Phase 6C-O |
| Darwin arm64 — Phase 6C-K Shadowsocks 2022 ChaCha8 TCP | Oracle | **Parity in declared scope** | Native exact-key config and official-library TCP differential passed for domain/large/half-close relay, 2026-08-29; ChaCha8 UDP stays rejected |
| Darwin arm64 — Phase 6C-L Shadowsocks IPv6 UDP | Oracle | **Parity in declared scope** | Native AEAD, stream and extra-AEAD representatives pass mixed/SOCKS5 explicit-IPv6 UDP relay and process-survival comparison, 2026-08-29 |
| Darwin arm64 — Phase 6C-M1 Shadowsocks simple-obfs HTTP | Oracle | **Parity in declared scope** | Native custom/default Host config and independent HTTP-unwrapper differential pass domain/128 KiB TCP, process survival and the oracle's half-close limitation, 2026-08-29 |
| Darwin arm64 — Phase 6C-M2 Shadowsocks simple-obfs TLS | Oracle | **Parity in declared scope** | Native custom/default SNI config and independent ClientHello/record differential pass domain/128 KiB TCP, process survival and the oracle's half-close limitation, 2026-08-29 |
| Darwin arm64 — Phase 6C-M3 Shadowsocks v2ray-plugin WebSocket | Oracle | **Parity in declared scope** | Native plaintext `mux: false` differential through an Axum authority passes exact Host/path, domain/128 KiB TCP, process survival and the oracle's half-close limitation, 2026-08-29 |
| Darwin arm64 — Phase 6C-M4 Shadowsocks v2ray-plugin WebSocket TLS | Oracle | **Parity in declared scope** | Native non-mux WSS differential through a Rustls/Axum authority passes custom-root trust, skip verification, untrusted rejection, SNI/Host/path, domain/128 KiB TCP and lifecycle evidence, 2026-08-29 |
| Darwin arm64 — Phase 6C-M5 complete Shadowsocks v2ray-plugin TCP | Oracle | **Parity in documented functional scope** | Unified native differential passes headers/Host override, default mux, early data, raw/fast HTTP Upgrade, verification-name override, DER pin, mTLS, inline ECH and `proxy-server-nameserver` DNS ECH; >65,535-byte single-write corrupt-frame behavior is an explicit safety divergence, 2026-08-29 |
| Darwin arm64 — Phase 6C-M6 Shadowsocks shadow-tls v3 | Oracle | **Partial — wire parity in declared scope** | Native hand-rolled v1/v2/v3 client with rustls session-id hook passes Clash `plugin: shadow-tls` config contract and SS2022 TCP domain/large-payload wire comparison against the Go oracle; Chrome `client-fingerprint` is partial (10 vs 16 cipher suites); Linux CI runs `phase6c_shadowtls_clienthello_regression.py` as per-runtime partial-Chrome regression on production v3 capture paths (not Go/Rust wire parity); process survival matches and half-close shares the oracle limitation, 2026-08-29 |
| Darwin arm64 — Phase 6C-N Shadowsocks inbound | Oracle | **Parity in declared first-server scope** | Corrected `phase6c_shadowsocks_inbound.py` passes the full Go/Rust ss-config/named TCP/UDP, UoT, obfs, ShadowTLS proxy/INNER/fallback, shutdown and identity-changing reload comparison after the protocol ownership refactor, 2026-09-01; Rust-only ShadowTLS `IN-USER` and EIH observations are excluded from parity |
| Darwin arm64 — Phase 6C-O Shadowsocks 2022 UDP outbound | Oracle | Pending | Default controller-outbound shard runs `phase6c_shadowsocks_2022_udp.py` for the three standard 2022 methods plus AES EIH; no native Darwin result is claimed before completion |
| Darwin arm64 — Phase 6D-A VMess AEAD native TCP | Oracle | **Parity in declared scope** | Native `phase6d_vmess_tcp.py` passes top-level/provider routing, independently decoded domain/IPv4 destinations, small/large records, half-close, controller fields and failure lifecycle, 2026-08-31 |
| Darwin arm64 — Phase 6D-B VMess explicit AEAD | Oracle | **Parity in declared scope** | Native `phase6d_vmess_aead.py` passes AES-128-GCM/ChaCha20-Poly1305 × plain/global-padding/authenticated-length/combined framing, independently decoded domain/IPv4/IPv6 destinations, 128 KiB multi-record relay and half-close, 2026-09-01 |
...
| Darwin arm64 — Phase 6D-I VMess gRPC/Gun | Oracle | **Parity in declared scope** | Native `phase6d_vmess_grpc.py` passes default/named/custom-path Gun services over h2c/H2, exact Host/path/content-type/User-Agent and envelope lengths, zero/positive AlterID, raw/AEAD/CFB representatives, 128 KiB relay and process survival, 2026-09-01 |
| Darwin arm64 — Phase 6D-J VMess gRPC/Gun pool | Oracle | **Parity in declared scope** | Native `phase6d_vmess_grpc_pool.py` passes default one-connection concurrent reuse, `max-connections`/`min-streams` balancing, `max-streams`, signed controls, raw h2c PING observation, post-ping connection reuse and process survival, 2026-09-01 |
| Darwin arm64 — Phase 6D-K VMess mKCP TCP | Oracle | **Parity in declared scope** | Native `phase6d_vmess_mkcp_mekya.py` passes authentication/headers/relay plus 14% and 25% semantic DATA loss with congestion off/on, ACK loss, duplication, reordering, bounded retransmission, 10/10 oracle-negative client half-close and 3/3 peer payload+EOF close, 2026-09-02 |
| Darwin arm64 — Phase 6D-L VMess Mekya TCP | Oracle | **Parity in declared scope** | The same native differential passes TLS Mekya over negotiated H2 and HTTP/1.1 with bundles/polling/H2 pool/nested mKCP, plus 10/10 oracle-negative client half-close and 3/3 peer payload+EOF close for both HTTP versions, 2026-09-02 |
| Darwin arm64 — Phase 6E-A VLESS native TCP | Oracle | **Parity in declared scope** | Native `phase6e_vless_tcp.py` passes independently parsed exact v0 requests, canonical/mapped UUIDs, domain/IPv4/IPv6, addon response, 128 KiB relay, half-close, provider/group/controller and failure lifecycle, 2026-09-02 |
| Darwin arm64 — Phase 6E-B VLESS native TCP over TLS | Oracle | **Parity in declared scope** | Native `phase6e_vless_tls.py` passes Python-TLS authority trust/name/SNI/skip checks, untrusted and wrong-name rejection, 128 KiB relay, half-close, controller group health and lifecycle, 2026-09-02 |
| Darwin arm64 — Phase 6E-C–E VLESS stream transports | Oracle | **Parity in declared scope** | Native differentials cover WS/WSS, HTTP/H2 and single-stream gRPC/Gun with configured/default request metadata, TLS policy, large relay and lifecycle evidence, 2026-09-02 |
| Darwin arm64 — Phase 6E-F/I VLESS UDP/XUDP | Oracle | **Parity in declared common-carrier scope** | Native differentials cover packet-address/XUDP associations over plaintext TCP, native TLS, plaintext WS and TLS gRPC/Gun with exact carrier metadata and process survival, 2026-09-03 |
| Darwin arm64 — Phase 6E-G VLESS Vision | Oracle | **Parity in declared scope** | Padding/end framing, fragmented response, half-close, controller HTTPS health measurement and real nested-TLS traffic pass through the record-bounded raw-TCP DIRECT splice, 2026-09-02 |
| Darwin arm64 — Phase 6E-H VLESS REALITY | Oracle | **Parity in declared Chrome 133 scope** | Authenticated default and X25519+ML-KEM-768 handshakes, relay/half-close and the complete normalized 16-cipher/18-extension ClientHello semantic differential pass, 2026-09-02 |
| Darwin arm64 — Phase 6E-J VLESS REALITY + Vision | Oracle | **Parity in declared scope** | Native small/large relay, half-close and nested TLS 1.3 pass through the authenticated, record-bounded REALITY-to-Vision DIRECT transition, 2026-09-03 |
| Darwin arm64 — Phase 6E-K VLESS xHTTP | Oracle | **Parity in basic stream-one scope** | Native TLS/H2 differential passes Go-compatible trailing-slash path, Host, custom/content-type headers, fixed padding, small/128-KiB relay and process survival, 2026-09-03 |
| Darwin arm64 — Phase 6E-L–N VLESS production gates | Oracle | **Parity in declared scope** | Native differentials pass pooled Gun controls/reconnect, xHTTP common modes/basic XMUX/REALITY, real-sing-vless 32/16-stream pressure, 16 repeated failures with recovery, and bounded malformed-response corpus, 2026-09-03 |
...
| Linux amd64 — Phase 6A1 built-ins/GLOBAL | Oracle | Pending | The isolated built-ins shard enables the real 60-second REJECT-DROP expiry comparison; no result is claimed before completion |
...
| Linux amd64 — Phase 6B3 HTTP/SOCKS5 completion | Oracle | Pending | Default controller-outbound shard includes TLS identity and plaintext/TLS SOCKS5 UDP completion gates; no native result is claimed before completion |
| Linux amd64 — Phase 6C-A Shadowsocks AES-128-GCM TCP | Oracle | Pending | Default controller-outbound shard includes the official-library authority differential; no native result is claimed before completion |
| Linux amd64 — Phase 6C-B Shadowsocks SIP004 AEAD TCP | Oracle | Pending | Default controller-outbound shard includes the three-cipher address/framing/lifecycle differential; no native result is claimed before completion |
| Linux amd64 — Phase 6C-C Shadowsocks SIP004 AEAD UDP | Oracle | Pending | Default controller-outbound shard includes the three-cipher mixed/SOCKS5 UDP differential; no native result is claimed before completion |
| Linux amd64 — Phase 6C-D Shadowsocks local providers/groups | Oracle | Pending | Default controller-outbound shard includes inline/file provider, selector and TCP/UDP relay evidence; no native result is claimed before completion |
| Linux amd64 — Phase 6C-E Shadowsocks HTTP provider/health | Oracle | Pending | Default controller-outbound shard includes HTTP download/refresh/cache, TCP/UDP replacement and SS health-probe evidence; no native result is claimed before completion |
| Linux amd64 — Phase 6C-F Shadowsocks UDP-over-TCP | Oracle | Pending | Default controller/outbound shard includes config/version, controller capability and UoT v1/v2 encrypted TCP relay evidence; no native result is claimed before completion |
| Linux amd64 — Phase 6C-G Shadowsocks legacy stream ciphers | Oracle | Pending | Default controller/outbound shard includes all eight methods over TCP/native UDP; no native result is claimed before completion |
| Linux amd64 — Phase 6C-H Shadowsocks extra AEAD ciphers | Oracle | Pending | Default controller/outbound shard includes all five methods over TCP/native UDP; no native result is claimed before completion |
| Linux amd64 — Phase 6C-I Shadowsocks 2022 TCP | Oracle | Pending | Default controller/outbound shard includes key validation and all three standard TCP methods; no native result is claimed before completion |
| Linux amd64 — Phase 6C-J Shadowsocks 2022 EIH | Oracle | Pending | Default controller/outbound shard includes AES-128/256 single-hop EIH validation and TCP wire gates; no native result is claimed before completion |
| Linux amd64 — Phase 6C-K Shadowsocks 2022 ChaCha8 TCP | Oracle | Pending | Default controller/outbound shard includes exact-key validation and TCP wire gates; no native result is claimed before completion |
| Linux amd64 — Phase 6C-L Shadowsocks IPv6 UDP | Oracle | Pending | Default controller/outbound shard includes AEAD/stream/extra-AEAD IPv6 UDP wire gates; no native result is claimed before completion |
| Linux amd64 — Phase 6C-M1 Shadowsocks simple-obfs HTTP | Oracle | Pending | Default controller/outbound shard includes config, exact Host and independent HTTP-unwrapper TCP/lifecycle gates; no native result is claimed before completion |
| Linux amd64 — Phase 6C-M2 Shadowsocks simple-obfs TLS | Oracle | Pending | Default controller/outbound shard includes config, exact SNI and independent ClientHello/record TCP/lifecycle gates; no native result is claimed before completion |
| Linux amd64 — Phase 6C-M3 Shadowsocks v2ray-plugin WebSocket | Oracle | Pending | Default controller/outbound shard includes plaintext non-mux config, exact Host/path and Axum-authority TCP/lifecycle gates; no native result is claimed before completion |
...
| Linux amd64 — Phase 6C-N Shadowsocks ss-config inbound | Oracle | Pending | `local-runtime` runs the corrected `phase6c_shadowsocks_inbound.py`, including proxy-observed leaf/group `handshake.proxy`, destination-discriminating `IN-TYPE,INNER`, an active fallback probe and identity-changing reload. Rust-only `IN-USER`/EIH are excluded from parity; no native result is claimed before completion |
| Linux amd64 — Phase 6C-O Shadowsocks 2022 UDP outbound | Oracle | **Local evidence; Actions confirmation pending** | Local `phase6c_shadowsocks_2022_udp.py` passed on Linux amd64, 2026-09-11: mixed/SOCKS5 IPv4/IPv6/domain, AES EIH, native association, controller `udp`/`uot`, wrong-key timeout, concurrent clients and server restart match the pinned Go oracle. ChaCha8 UDP and 2022 UoT remain rewrite-only rejections. GitHub Actions still owns the recorded Linux shard result |
| Linux amd64 — Phase 6D-A VMess AEAD native TCP | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_tcp.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-B VMess explicit AEAD | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_aead.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-C VMess native-TCP security modes | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_security.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-D VMess legacy AlterID native TCP | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_alterid.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-E VMess UDP packet modes | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_udp.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-F VMess TLS and WebSocket TCP | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_websocket.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-G VMess WebSocket variants | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_websocket_variants.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-H VMess HTTP transports | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_http.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-I VMess gRPC/Gun | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_grpc.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-J VMess gRPC/Gun pool | Oracle | Pending | Default controller/outbound shard runs `phase6d_vmess_grpc_pool.py`; no native result is claimed before completion |
| Linux amd64 — Phase 6D-K VMess mKCP TCP | Oracle | Pending | Default controller/outbound shard runs authentication/header/relay, semantic moderate/heavy impairment, congestion off/on, ACK loss, duplication/reordering and both close-direction gates; no native result is claimed before completion |
| Linux amd64 — Phase 6D-L VMess Mekya TCP | Oracle | Pending | Default controller/outbound shard runs TLS H2/HTTP1 bundle/poll/pool/nested-mKCP plus both close-direction gates; no native result is claimed before completion |
| Linux amd64 — Phase 6E-A VLESS native TCP | Oracle | Pending | Default controller/outbound shard runs `phase6e_vless_tcp.py`, including exact independent-authority framing, UUID mapping, relay, integration and lifecycle gates; no native result is claimed before completion |
| Linux amd64 — Phase 6E-B VLESS native TCP over TLS | Oracle | Pending | Default controller/outbound shard runs `phase6e_vless_tls.py`, including trust/name/SNI/skip, negative verification, relay, health and lifecycle gates; no native result is claimed before completion |
| Linux amd64 — Phase 6E-C–F/I VLESS transports and UDP | Oracle | Pending | Default controller/outbound shard runs WS/WSS, HTTP/H2, gRPC/Gun, native UDP/XUDP and common UDP-carrier differentials; no native result is claimed before completion |
| Linux amd64 — Phase 6E-G/H/J Vision and REALITY | Oracle | Pending | Default CI runs nested-TLS Vision DIRECT, REALITY default/hybrid, normalized Chrome 133 ClientHello and REALITY+Vision differentials; no native Linux result is claimed before completion |
| Linux amd64 — Phase 6E-K VLESS xHTTP | Oracle | Pending | Default CI runs the TLS/H2 basic `stream-one` differential; no native Linux result is claimed before completion |
| Linux/Windows/macOS CI — Phase 6E-L–N VLESS gates | Oracle | Pending | A dedicated `vless` shard runs every Phase 6E script, including pool, xHTTP and bounded production gates, independently of older controller/outbound regressions; no platform result is claimed before completion |
...
| Windows amd64 — Phase 8C TUN | Oracle | **Partial** (data path implemented; native pass unclaimed until CI) | Wintun load (`MIHOMO_WINTUN` scratch extract for Rust / next-to-exe for Go, official 0.14.1 zip hashed in `phase8c_tun.py`; WinError 32 reuses a matching digest only, otherwise isolates Go+DLL), `delete_driver(false)`, this-adapter metric 1, Linux-style split auto-route, TUN-adapter-only DNS, existing-adapter refusal. Privileged `PHASE8C_NATIVE=1` / `phase8c-windows-tun` is fail-closed. Not Parity |
| Linux amd64 — Phase 8F TUN network-change | Oracle | **Partial** (monitor implemented; native pass unclaimed until CI) | Default-route poll excluding TUN (IPv4 and IPv6 separately), snapshot re-protect for captured families (never via TUN; uncaptured IPv6 skipped), refuse-foreign exact prefixes, socket-level DIRECT/TUIC/Hysteria2 bind (no DIRECT host route; Windows TCP local port 0), QUIC clients retired and DIRECT UDP rebound on NIC change, matching-family auto-detect bind (pure IPv6 is not lost; captured family without physical egress is refused), loopback UDP remotes skip NIC bind, `route-exclude-address` physical exceptions, resolver `reset_connections()` without fake-IP flush, 4096 TCP/UDP caps, bounded UDP reply queue, 1024 host-route cap fails closed. Privileged dual-uplink `PHASE8F_NATIVE=1` / `phase8f-linux-tun` is fail-closed and covers flap with the old uplink down, public DIRECT, same-IP REJECT, domain TUIC, same-socket DIRECT UDP, mixed/SOCKS UDP to localhost, and foreign-route conflict. Native Darwin/Windows NIC flap, UDP fragment/loss and TUN TCP half-close/RST are not in this gate. Not Parity |
