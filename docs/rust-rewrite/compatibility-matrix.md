...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–C adds top-level and file-provider/selector VMess records with fail-closed validation for every AlterID-zero native-TCP security mode. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VMess | Oracle | **Parity in Phase 6D-A–C AlterID-zero native-TCP scope** | `auto`, AES-128-GCM, ChaCha20-Poly1305, `none`/`zero` and AES-128-CFB pass three independent-authority differentials; AEAD modes cover every padding/authenticated-length combination, while non-AEAD modes reproduce the oracle's ignored wire flags. Evidence includes domain/IPv4/IPv6, small/128 KiB relay, half-close, failure lifecycle, controller shape and file-provider selector routing. UDP/XUDP, nonzero AlterID/legacy headers, TLS, transports, mux and server direction remain open |
