...
| Full DIRECT TCP/UDP/platform options | Oracle | Partial | Phase 1/3 local TCP plus SOCKS UDP; Phase 5F applies interface, Linux/Android routing mark, keepalive and concurrent-domain policy to DIRECT/current HTTP/SOCKS5 dials, plus TFO/MPTCP on fixed inbound TCP. Bounded IPv4/IPv6 NAT, fan-out, pressure recovery and generation retention pass; privileged nonzero-mark, CI timeout result and remote UDP adapters remain unclaimed |
| REJECT / REJECT-DROP | Oracle | **Partial** | `phase6a_builtins.py` proves immediate REJECT and REJECT-DROP retention before/after payload; Linux CI additionally enables the oracle's 60-second expiry gate. Exact expiry and UDP behavior remain unclaimed until their gates complete |
...
| Configured DNS / REMATCH adapters | Oracle | Partial | Phase 5B3g proves configured REMATCH metadata/rescan behavior; DNS adapter transport, adapter views, configured simple-adapter validation and UDP remain later Phase 6A gates |
