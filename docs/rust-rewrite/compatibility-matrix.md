...
| WireGuard / AmneziaWG | Oracle | **Partial — 6I-A single-peer IPv4 TCP outbound** | YAML `type: wireguard` userspace Noise (`defguard_boringtun`) + smoltcp TCP stack, mixed/SOCKS TCP, rules/groups/providers/health/reload in `phase6i_wireguard_tcp.py`. No OS TUN / admin for mixed. AmneziaWG, `peers`, inner IPv6, remote DNS, UDP relay and inbound remain rejected/deferred. Not Parity; three-platform CI pending |
