...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-B adds top-level `ss` with required server/port/password and all three standard SIP004 AEAD TCP ciphers plus its controller view. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Shadowsocks (`ss`) | Oracle | **Parity in Phase 6C-C SIP004 AEAD TCP/UDP scope** | `phase6c_shadowsocks.py`, `phase6c_shadowsocks_ciphers.py` and `phase6c_shadowsocks_udp.py` run Go and Rust through the same official-library SS authority. AES-128-GCM, AES-256-GCM and ChaCha20-IETF-Poly1305 pass domain/IPv4 TCP and UDP relay; TCP additionally covers 128 KiB framing and half-close, while each mixed and SOCKS5 UDP client reuses its association and matches the `udp: true`/`uot: false` controller boundary. Stream/extra/2022 ciphers, UoT, plugins, providers/groups, server direction, IPv6 UDP and advanced socket/dialer options remain open |
