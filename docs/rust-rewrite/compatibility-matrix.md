...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-E covers top-level plus inline/file/HTTP-provider `ss` with the three standard SIP004 AEAD ciphers, optional UDP and controller views. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Shadowsocks (`ss`) | Oracle | **Parity in Phase 6C-E native SIP004 AEAD scope** | The Phase 6C A–C differentials prove all three accepted ciphers over domain/IPv4 TCP and UDP, TCP framing/half-close, mixed/SOCKS5 UDP association reuse and controller capability. Phase 6C-D adds inline/file provider selection; `phase6c_shadowsocks_http_provider.py` proves HTTP download, A→B manual refresh, TCP/UDP record replacement, fresh-cache restart and real health probing through SS TCP. Stream/extra/2022 ciphers, UoT, plugins, server direction, IPv6 UDP, provider scheduling/ETag stress and advanced socket/dialer options remain open |
