...
| LAN allow/deny and skip-auth prefixes | Oracle | **Parity in declared IPv4 fixed-listener scope** | `phase5f_lan_policy.py` proves wildcard/explicit bind, allow-before-deny filtering, auth bypass, loopback override and `/configs` fields across fixed HTTP/SOCKS/mixed; IPv6, non-loopback native reachability and bind-address hot rebind remain |
...
| `/configs` GET/PUT/PATCH and `/configs/geo` | Oracle | Partial | Phase 5D proves executable config transactions and path rollback; Phase 5E adds exact `POST /configs/geo` 204, replacement and malformed-data rollback for current geodata consumers; Phase 5F1 adds LAN bind/filter/skip-auth snapshot fields. Unimplemented config runtimes and broader consumers remain feature gates |
...
| Darwin arm64 — Phase 5F1 LAN policy | Oracle | **Parity in declared IPv4 scope** | Native fixed HTTP/SOCKS/mixed bind, allow/deny, skip-auth and controller snapshot differential passed, 2026-08-28; IPv6/rebind/TFO/MPTCP remain |
...
| Linux amd64 — Phase 5F1 LAN policy | Oracle | Pending | Dedicated parallel local-runtime shard includes the fixed-listener LAN policy differential; no result is claimed before completion |
| Linux amd64 — Phase 5F2 UDP NAT | Oracle | Pending | Dedicated parallel local-runtime shard includes the deterministic UDP NAT differential; no result is claimed before completion |
