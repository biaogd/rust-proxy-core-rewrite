...
| dialer-proxy TCP chains | Oracle | **Partial — 7T1-A** | Shared TCP dial entry; B with `dialer-proxy: A` reaches B's server via A. HTTP↔SOCKS5 path-proving differentials pass (`phase7t1_dialer_proxy_tcp.py`); SS/SSR/Snell also dial through the shared entry. VMess/VLESS/Trojan/AnyTLS dialer-proxy wiring is next. Missing/self/cycle refs fail closed. SSH/Hy2/TUIC/WG `dialer-proxy`, UDP chains and sing-mux remain rejected. See `OUT-21-dialer-proxy-chains.md` |
