...
| VMess | Oracle | **Parity in Phase 6D-A–L declared client scope** | A–J retain the security, AlterID, UDP, TLS/WS/HTTP/H2 and pooled Gun evidence. K adds VMess TCP over V2Ray mKCP with default/simple authentication, AES-GCM seed and SRTP/uTP/WeChat/DTLS/WireGuard headers. L adds TLS Mekya over both negotiated H2 and HTTP/1.1, packet bundles, polling and H2 pool selection. Loss/congestion stress, transport half-close, UDP over non-native transports, advanced TLS, xHTTP/H3, general TCP mux and server direction remain open |
...
| Shared outbound transport/security variants | Oracle | Partial | Phase 6D-F–J covers TLS/WS/Upgrade/HTTP/H2 and pooled Gun. Phase 6D-K adds the custom V2Ray mKCP segment/auth/header transport rather than substituting generic KCP; 6D-L layers mKCP over a Hyper-backed Mekya HTTP/1.1/H2 packet carrier. Current K/L evidence is VMess TCP on deterministic loopback; xHTTP/H3, loss/congestion stress, other consumers, Reality, ECH, JLS, ReSTLS, ShadowTLS and TLSMirror remain separate gates |
...
| Darwin arm64 — Phase 6D-K VMess mKCP TCP | Oracle | **Parity in declared scope** | Native `phase6d_vmess_mkcp_mekya.py` passes default/simple authentication, AES-GCM seed, all five camouflage headers, small/128 KiB VMess TCP relay, independent Go authority parsing and process survival, 2026-09-01 |
| Darwin arm64 — Phase 6D-L VMess Mekya TCP | Oracle | **Parity in declared scope** | The same native differential passes TLS Mekya over negotiated H2 and HTTP/1.1 with request packet bundles, polling, H2 pool size two, nested seeded/header mKCP, small/128 KiB relay and process survival; oracle transport half-close is excluded, 2026-09-01 |
...
| Linux amd64 — Phase 6D-K VMess mKCP TCP | Oracle | Pending | Default controller/outbound shard runs the default/seed/all-header and small/128 KiB independent-authority differential; no native result is claimed before completion |
| Linux amd64 — Phase 6D-L VMess Mekya TCP | Oracle | Pending | Default controller/outbound shard runs TLS H2/HTTP1 packet-bundle, polling, pool and nested-mKCP evidence; no native result is claimed before completion |
