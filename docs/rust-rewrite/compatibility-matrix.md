...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–J adds top-level and file-provider/selector VMess records with fail-closed validation for every security mode, positive legacy AlterID, standard/packet-address/XUDP UDP selection, TLS/WS/WSS variants, HTTP/1/H2 and pooled gRPC/Gun TCP transports. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Named listeners | Oracle | Partial; corrected 6C-N differential pending | Phase 6C-N implements named `shadowsocks` listeners with simple-obfs, shadow-tls v3, 2022 TCP/EIH, UoT and identity-changing reload. Corrected gates require proxy-observed `handshake.proxy`, discriminating `IN-TYPE,INNER` and fail-closed fields; ShadowTLS `IN-USER` and EIH inbound are Rust-only evidence. Common listener fields and remaining listener types stay open |
...
| Shadowsocks | Yes | Yes | Oracle | Partial; corrected 6C-N differential pending | Phase 6C-N implements the roadmap's declared ss-config/named TCP and UDP first-server slice. Focused corrected Rust proxy/INNER/shutdown/reload scenarios pass; the corrected full Go/Rust `phase6c_shadowsocks_inbound.py` result, full inbound cipher matrix and remaining server features stay open |
...
| Shadowsocks (`ss`) | Oracle | **Partial — Phase 6C-M6 client wire parity in declared TCP scope; 6C-N inbound validation pending** | A–H proves eight shared SIP004 AEAD plus eight legacy stream methods over TCP/native UDP, providers/health and UoT v1/v2. I/J proves three standard 2022 methods and AES single-hop EIH over TCP; K adds ChaCha8-2022 TCP. L proves shared native UDP relay to explicit IPv6 destinations. M1/M2 adds simple-obfs HTTP/TLS; M5 closes documented v2ray-plugin TCP options; M6 adds native `plugin: shadow-tls` v1/v2/v3 over SS2022 TCP. M6 Chrome remains partial at 10 Rust cipher suites versus Go/uTLS 16, and its ClientHello regression is per-runtime rather than wire parity. Phase 6C-N separately implements the first inbound/server slice. Native 2022 UDP, multi-hop EIH, Go-only methods, advanced socket/dialer options and inbound features outside 6C-N remain open |
| ShadowsocksR (`ssr`) | Oracle | Not started | Cipher/protocol/obfs interop |
| VMess | Oracle | **Parity in Phase 6D-A–J declared client scope** | Every supported security mode passes independent-authority differentials with AEAD and positive legacy AlterID headers. Phase 6D-E proves ordinary/packet-address/XUDP UDP; F/G prove TLS, WS/WSS early data and raw Upgrade; H proves HTTP/HTTPS first-write obfuscation plus h2c/H2 PUT streams; I proves Gun metadata/envelopes; J proves reusable H2 connections, concurrent stream thresholds and health PING. Negative AlterID, oversized UDP edges, packet-address FQDN payloads, UDP over non-native transports, exact frame-idle ping timing, advanced TLS, remaining outer transports, general TCP mux and server direction remain open |
...
| Shared outbound transport/security variants | Oracle | Partial | Phase 6D-F/G reuses the existing Rustls, Tungstenite and validated raw-Upgrade boundaries for VMess TLS/WS/WSS TCP, including lazy early data and write-before-response fast open. Phase 6D-H adds a focused HTTP first-write adapter and library-backed `h2` client stream with TLS/h2c; 6D-I/J layers Go-compatible Gun envelopes, request metadata, pooled concurrent streams and health PING over reusable H2 connections. xHTTP/H3, mKCP, Mekya, Reality, ECH, JLS, ReSTLS, ShadowTLS and TLSMirror remain per-consumer gates |
...
| Darwin arm64 — Phase 6C-N Shadowsocks inbound | Oracle | Implemented; corrected full differential pending | Workspace fmt/clippy/test and five focused corrected Rust proxy-observation, INNER, shutdown and identity-changing reload scenarios pass at `c4ec1e4a`, 2026-08-31. The local full runner stopped at the pre-existing first Go SS handshake before those cases, so parity is not claimed |
