...
| Trojan | Yes | Yes | Oracle | **Partial — IN-C TLS + WSS + gRPC inbound** | Named `type: trojan` TLS, `ws-path` WSS, and `grpc-service-name` Gun TCP+UDP-over-TLS via `phase_inc_trojan_tls.py` / `phase_inc_trojan_websocket.py` / `phase_inc_trojan_grpc.py`; combined ws+grpc / Reality / `ss-option` rejected; outbound remains Phase 6F |
