...
| Proxy providers and health checks | Oracle | Partial | Local YAML file load, views, selected routing, manual transactional refresh/rollback and filtered composition pass. Plaintext HTTP now passes bounded initial/manual/interval fetch, repeated headers, ETag enable/disable and 304, size rejection, explicit and URL-MD5 default cache paths, fresh restart reuse, stale-start refresh, corrupt-cache recovery, failed-refresh old-data retention, SIGHUP interval/source/cache mutation, atomic group switching and rollback; HTTPS, ETag restart persistence and real provider health remain |
...
| `/proxies`, `/group`, delay and selection | Oracle | Partial | `phase5d_proxies.py` proves all seven built-ins and GLOBAL; Phase 5C adds nested configured selectors, reload reconciliation, filtered/include-all provider composition, empty fallback and live recursive routing; selector fixtures isolate HOME/XDG/Mihomo profile state so persisted choices cannot cross scenarios; automatic groups, HTTPS and exhaustive failure/persistence remain unclaimed |
...
| Proxy and rule provider APIs | Oracle | Partial | File provider views/composition pass; `phase5c_provider_refresh.py` proves PUT replacement and rollback. Phase 5C2c–2h add plaintext HTTP views, initial/PUT/interval refresh, headers, ETag 304, size bounds, default cache/restart reuse, stale/corrupt-cache recovery, SIGHUP schedule/source mutation, selected replacement routing, atomic cache replacement and 503 rollback; HTTPS, ETag restart persistence, concurrent/coalesced refresh, real health state, rule-provider detail and broader persistence remain unclaimed |
