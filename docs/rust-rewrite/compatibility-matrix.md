...
| Trojan | Oracle | **Partial — Phase 6F-A–C client scope** | A proves native TLS TCP authentication/framing/relay; B proves native TLS UDP framing, splitting and reuse; C proves TCP and UDP over WSS with path, Host/custom headers and WebSocket ALPN. TLS name override/skip still rely on shared transport evidence; gRPC, REALITY, fallback and server direction remain open |
