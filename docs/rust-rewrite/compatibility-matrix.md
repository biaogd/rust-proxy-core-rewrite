...
| Snell | Oracle | **Partial — 7E-A/B/C versions 1–3 TCP + v3 UDP + simple-obfs** | YAML `type: snell` `psk` + version 1/2/3 through mixed/SOCKS TCP in `phase7e_snell_tcp.py`; v3 `udp: true` through mixed/SOCKS UDP ASSOCIATE in `phase7e_snell_udp.py`; `obfs-opts` HTTP/TLS in `phase7e_snell_obfs.py`. v1 ChaCha20-Poly1305, v2/v3 AES-128-GCM, Argon2id KDF. `reuse`/remaining `obfs-opts`/v4/v5/`dialer-proxy` and inbound remain rejected. Not Parity; three-platform CI is the new snell shard |
