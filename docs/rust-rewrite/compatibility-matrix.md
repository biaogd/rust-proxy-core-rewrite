...
| LAN allow/deny, skip-auth and fixed-listener socket policy | Oracle | **Parity in declared fixed-listener scope** | `phase5f_lan_policy.py` proves IPv4/IPv6 wildcard and explicit bind, native non-loopback reachability, same-port IPv4→IPv6 PATCH rebind, filtering/auth rollback, invalid-interface behavior and nonzero-mark `/configs` fields; TFO uses `tokio-tfo`, Linux MPTCP falls back to TCP and keepalive is library-backed, while privileged global-unicast mark evidence remains pending |
...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–L adds the declared VMess surface. Phase 6E-A–K adds VLESS native/TLS/WS/HTTP/H2/gRPC, UDP/XUDP over the common carriers, Vision, REALITY, REALITY+Vision and basic xHTTP `stream-one`, with unsupported xHTTP modes rejected. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VLESS | Yes | Transport-dependent | Oracle | Partial | Phase 6E-A–K client scopes; inbound/server direction remains open |
...
| VLESS | Oracle | **Parity in Phase 6E-A–K declared client scopes** | A/B cover v0 native TCP/TLS; C–E add WS/WSS, HTTP/H2 and single-stream gRPC/Gun; F/I cover packet-address/XUDP plus native TLS, WS and TLS gRPC carriers. G/H/J prove Vision, REALITY and their native-TCP Chrome-133 composition. K adds TLS/H2 xHTTP `stream-one` with Host/path/headers/padding. Encryption extensions, UDP over HTTP/H2/xHTTP, WSS as a separate UDP gate, advanced xHTTP, pooled gRPC, other fingerprints, legacy-only TLS 1.2 suites, general mux and server direction remain open |
...
| Shared outbound transport/security variants | Oracle | Partial | Phase 6D-F–L covers TLS/WS/Upgrade/HTTP/H2, pooled Gun, mKCP and Mekya for VMess. Phase 6E-I/J/K add VLESS UDP carrier composition, REALITY+Vision and basic TLS/H2 xHTTP `stream-one`. Advanced xHTTP/H3, randomized long-duration mKCP impairment, other consumers, ECH, JLS, ReSTLS and TLSMirror remain separate gates |
