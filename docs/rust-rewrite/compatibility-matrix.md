...
| TUIC | Yes | Yes | Oracle | Partial | 6H-A v5 outbound TCP vs Go inbound in `phase6h_tuic_tcp.py`; UDP/lifecycle 6H-B/C, v4/0-RTT/inbound deferred |
...
| TUIC | Oracle | **Partial — 6H-A v5 TCP outbound** | YAML/UUID/password, QUIC/TLS exporter auth, TCP relay, groups/providers/health/reload, auth/cert/target-failure isolation in `compat/scripts/phase6h_tuic_tcp.py`. UDP native/stream relay is 6H-B; reuse/heartbeat command/soak is 6H-C. v4, 0-RTT, inbound, ECH and UDP-over-stream remain rejected/deferred |
