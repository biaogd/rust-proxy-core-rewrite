...
| Trojan | Oracle | **Partial — Phase 6F-A/B native TLS client scope** | A proves SHA-224 password key/rejection, domain/IPv4/IPv6 SOCKS address framing, standard TLS custom-root/SNI/ALPN, payload relay and half-close. B proves command-3 UDP framing, 8192-byte split bounds, same-association destination changes and exact controller UDP/UoT/XUDP capabilities. TLS name override/skip still rely on shared transport evidence; WS/gRPC, REALITY, fallback and server direction remain open |
