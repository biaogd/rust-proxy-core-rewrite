...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M5 covers top-level plus provider `ss` parsing with eight shared SIP004 AEAD, eight legacy stream ciphers, four 2022 TCP methods, AES-2022 single-hop EIH, native pre-2022 IPv4/IPv6 UDP, UoT v1/v2, simple-obfs HTTP/TLS and the documented v2ray-plugin TCP option surface. Remaining adapter types/options and the exhaustive error corpus remain open |
