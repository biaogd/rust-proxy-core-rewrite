...
| Proxy providers and health checks | Oracle | Partial | Local YAML file load, views, selected routing, manual transactional refresh/rollback and filtered composition pass. Plaintext HTTP now passes bounded initial/manual/interval fetch, repeated headers, ETag enable/disable and 304, size rejection, explicit and URL-MD5 default cache paths, restart cache reuse, atomic group switching and rollback; HTTPS, ETag restart persistence and real provider health remain |
