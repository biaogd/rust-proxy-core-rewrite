...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-A adds top-level `ss` with required server/port/password and `aes-128-gcm` TCP-only validation plus its controller view. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Shadowsocks (`ss`) | Oracle | **Parity in Phase 6C-A AES-128-GCM TCP scope** | `phase6c_shadowsocks.py` runs Go and Rust through the same official-library SS authority and compares domain-target AEAD TCP relay, MATCH rejection and controller type/UDP/UoT fields. Other ciphers, UDP, UoT, plugins, providers/groups, server direction and advanced socket/dialer options remain open |
