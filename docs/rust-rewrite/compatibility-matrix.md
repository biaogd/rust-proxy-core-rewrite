...
| Trojan | Oracle | **Partial — Phase 6F-A–D client scope** | A proves native TLS TCP authentication/framing/relay; B proves native TLS UDP framing, splitting and reuse; C proves TCP and UDP over WSS with path, Host/custom headers and WebSocket ALPN; D proves TCP and UDP over HTTP/2 Gun/gRPC, service path, User-Agent and pooled client controls. TLS name override/skip still rely on shared transport evidence; REALITY, fallback and server direction remain open |
