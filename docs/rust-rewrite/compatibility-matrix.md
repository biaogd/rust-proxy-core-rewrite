...
| Trojan | Oracle | **Partial — Phase 6F-A native TCP+TLS client scope** | SHA-224 password key/rejection, domain/IPv4/IPv6 SOCKS address framing, standard TLS custom-root/SNI/ALPN, payload relay and half-close are owned by the first gate. TLS name override/skip still rely on shared transport evidence; UDP, WS/gRPC, REALITY, fallback and server direction remain open |
