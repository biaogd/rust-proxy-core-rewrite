...
| WireGuard / AmneziaWG | Oracle | **Partial — 6I-A/B single-peer TCP+UDP outbound** | YAML `type: wireguard` userspace Noise (`defguard_boringtun`) + smoltcp TCP/UDP stack, mixed/SOCKS TCP+UDP, inner IPv6, `remote-dns-resolve`, rules/groups/providers/health/reload in `phase6i_wireguard_tcp.py` / `phase6i_wireguard_udp.py`. No OS TUN / admin for mixed. AmneziaWG, `peers`, `ip-stack` and inbound remain rejected/deferred. Not Parity; 6I-C and three-platform CI pending |
