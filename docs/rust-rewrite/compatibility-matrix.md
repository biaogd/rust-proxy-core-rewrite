...
| `/proxies`, `/group`, delay and selection | Oracle | Partial | `phase5d_proxies.py` proves all seven built-ins, GLOBAL group list/detail and DIRECT/REJECT selection, exact mutation/errors, local-HTTP HEAD delay, expected/timeout validation, history/extra health state and GLOBAL delay map; configured remote adapters/groups, HTTPS, data-plane GLOBAL routing and exhaustive failure/reload/persistence behavior remain unclaimed |
