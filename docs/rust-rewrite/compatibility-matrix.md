...
| VMess | Oracle | **Parity in Phase 6D-A–L declared client scope (+ UDP carriers)** | A–J retain the security, AlterID, UDP, TLS/WS/HTTP/H2 and pooled Gun evidence. E2 adds ordinary/packet-address/XUDP over native TLS, plaintext WS, WSS and Gun/gRPC by reusing those carriers, with 5s setup timeout/shutdown cancel. K adds VMess TCP over V2Ray mKCP with default/simple authentication, AES-GCM seed, all camouflage headers, Go-contract fast-ACK/RTO/congestion state, deterministic 14%/25% semantic DATA loss, ACK loss, duplication/reordering and the oracle's full-close-on-half-close behavior. L adds TLS Mekya over negotiated H2 and HTTP/1.1 with the same close lifecycle. UDP over HTTP/H2/mKCP/Mekya, randomized long-duration impairment, advanced TLS, xHTTP/H3, general TCP mux and server direction remain open |
