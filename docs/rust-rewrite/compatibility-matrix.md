...
| `/proxies`, `/group`, delay and selection | Oracle | Partial | `phase5d_proxies.py` proves all seven built-ins, GLOBAL group list/detail and DIRECT/REJECT selection, exact mutation/errors, local-HTTP HEAD delay, expected/timeout validation, history/extra health state and GLOBAL delay map; `phase5d_modes.py` connects the built-in selector to current TCP/UDP routing; configured remote adapters/groups, HTTPS and exhaustive failure/reload/persistence behavior remain unclaimed |
...
| Proxy and rule provider APIs | Oracle | Partial | `phase5d_providers.py` proves the implicit `default` proxy provider list/detail, DIRECT/REJECT member views, no-op update/health operations, exact missing-resource errors and the empty rule-provider collection; configured file/HTTP vehicles, real refresh/health state, rule-provider detail and persistence remain unclaimed |
