...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-M6 covers the declared `ss` client surface; Phase 6D-A–G adds top-level and file-provider/selector VMess records with fail-closed validation for every security mode, positive legacy AlterID, standard/packet-address/XUDP UDP selection and TLS/WS/WSS TCP including early/raw-upgrade variants. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| VMess | Oracle | **Parity in Phase 6D-A–G declared client scope** | Every supported security mode passes independent-authority differentials with AEAD and positive legacy AlterID headers. Phase 6D-E proves ordinary/packet-address/XUDP UDP. Phase 6D-F proves native TLS, WS and WSS TCP. Phase 6D-G proves named-header/path/query WebSocket early data plus unframed ordinary/fast HTTP Upgrade over plaintext/TLS with exact wire placement and relay lifecycle. Negative AlterID, oversized UDP edges, packet-address FQDN payloads, UDP over WS, advanced TLS, remaining outer transports, general TCP mux and server direction remain open |
