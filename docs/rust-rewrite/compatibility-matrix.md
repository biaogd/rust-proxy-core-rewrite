...
| Proxies and built-in proxy insertion | Oracle | Partial | Phase 6A covers built-ins and configured simple adapters; Phase 6B covers HTTP/SOCKS5; Phase 6C-G covers top-level plus inline/file/HTTP-provider `ss` with three SIP004 AEAD and eight shared legacy stream ciphers, native UDP, UoT v1/v2 and controller views. Remaining adapter types/options and the exhaustive error corpus remain open |
...
| Shadowsocks (`ss`) | Oracle | **Parity in Phase 6C-G declared cipher/UoT scope** | Phase 6C A–C proves three SIP004 AEAD methods over TCP/native UDP; D/E adds providers and health; F proves UoT v1/v2; G proves eight shared legacy stream methods over domain/large/half-close TCP and IPv4/domain UDP. Go-only extra methods, 2022, plugins, server direction, IPv6 UDP, UoT connect mode, provider scheduling/ETag stress and advanced socket/dialer options remain open |
