...
| CFG-05 | Proxy/rule providers, vehicles, health checks, refresh, persistence and overrides | Partial: local file load/manual refresh plus initial plaintext HTTP fetch with explicit cache path | 5C2–5C4 |
...
| PROV-01 | Proxy providers: file/HTTP vehicles, parsing, override, refresh, health checks and persistence | Partial: local YAML file load/manual refresh plus bounded initial plaintext HTTP fetch, validated cache, REST views and group consumption | 5C2 HTTP/interval/override/persistence gates |
