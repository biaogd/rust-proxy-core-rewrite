...
| OUT-11 | Hysteria v1/v2: QUIC/fake TCP, obfs, congestion, UDP and PMTUD | Partial: HY2-A/B/C implement Clash `type: hysteria2` outbound TCP+UDP with HTTP/3 auth, TLS verify/skip, stock Quinn BBR (up/down unset), Brutal/CC-RX reconcile, Salamander, port hop, udp-mtu/windows, stress/netem/reload-removal, soak harness (CI short; 2h via env), session reuse, groups/providers/health/reload and Go/Rust differentials; `cwnd`/`bbr-profile`/Gecko/Realm/ECH rejected at parse | Full ≥2h soak on release cadence; Hysteria v1 and inbound remain open |
