...
| OUT-11 | Hysteria v1/v2: QUIC/fake TCP, obfs, congestion, UDP and PMTUD | Partial: HY2-A/B implement Clash `type: hysteria2` outbound TCP+UDP with HTTP/3 auth, TLS verify/skip, stock Quinn BBR (up/down unset), Brutal/CC-RX reconcile, Salamander, port hop, udp-mtu/handshake timeout/receive windows, session reuse, groups/providers/health/reload and Go/Rust differentials; `cwnd`/`bbr-profile`/Gecko/Realm/ECH rejected at parse | HY2-C stress/netem/soak; Hysteria v1 and inbound remain open |
