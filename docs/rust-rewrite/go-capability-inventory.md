...
| CFG-03 | Proxies, reserved built-ins, duplicate/reserved-name checks and dependency ordering | Partial: Phase 6A covers reserved/default GLOBAL, collisions and configured direct/reject/DNS/REMATCH validation/order; Phase 6B adds current HTTP/SOCKS5 dependencies; Phase 6D-A/B adds fail-closed native-TCP VMess records at top level and through file providers/selectors, including explicit AEAD/padding/authenticated-length options | Later protocol records, broader VMess options, dialer-proxy chains and their dependency ordering remain |
...
| OUT-07 | VMess: security/alter-id, TCP/UDP, early data and packet modes | Partial: Phase 6D-A/B implements `auto` plus explicit AES-128-GCM/ChaCha20-Poly1305, global padding/authenticated length, AlterID 0, native TCP, domain/IPv4/IPv6 targets and bounded relay/lifecycle behavior | UDP/XUDP/packet-address, `none`/`zero`/legacy AlterID/ciphers and every TLS/plugin/mux transport remain later 6D/7T gates |
