...
| CFG-03 | Proxies, reserved built-ins, duplicate/reserved-name checks and dependency ordering | Partial: Phase 6A covers reserved/default GLOBAL, collisions and configured direct/reject/DNS/REMATCH validation/order; Phase 6B adds current HTTP/SOCKS5 dependencies; Phase 6D-A–I adds fail-closed VMess records at top level and through file providers/selectors, including every security label, AEAD framing options, positive legacy AlterID, standard/packet-address/XUDP UDP modes, TLS/WS/WSS variants, HTTP/1/H2 and single-stream gRPC/Gun TCP transports | Later protocol records, negative AlterID, advanced TLS/remaining transport options, gRPC pool controls, dialer-proxy chains and their dependency ordering remain |
...
| OUT-07 | VMess: security/alter-id, TCP/UDP, early data and packet modes | Partial: Phase 6D-A–I implements every security label (`auto`, AES-128-GCM, ChaCha20-Poly1305, `none`/`zero`, AES-128-CFB), AEAD padding/authenticated length, zero and positive legacy AlterID headers, native TCP plus standard/packet-address/XUDP UDP, TLS/WS/WSS early/raw-Upgrade variants, HTTP/HTTPS/h2c/H2 and gRPC/Gun single-stream TCP | Negative AlterID, oversized UDP edges, packet-address FQDN payloads, UDP over non-native transports, advanced TLS and remaining outer/general-mux transports remain later 6D/7T gates |
