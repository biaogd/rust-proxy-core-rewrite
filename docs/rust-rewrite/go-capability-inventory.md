...
| CFG-05 | Proxy/rule providers, vehicles, health checks, refresh, persistence and overrides | Partial: local file load/manual refresh plus plaintext HTTP initial/manual/interval refresh with explicit cache path | 5C2–5C4 |
...
| PROV-01 | Proxy providers: file/HTTP vehicles, parsing, override, refresh, health checks and persistence | Partial: local YAML file load/manual refresh plus bounded plaintext HTTP initial/manual/interval refresh, validated cache, REST views and group consumption | 5C2 HTTPS/conditional/override/persistence gates |
