...
| CFG-03 | Proxies, reserved built-ins, duplicate/reserved-name checks and dependency ordering | Partial: Phase 6A covers reserved/default GLOBAL, collisions and configured direct/reject/DNS/REMATCH validation/order; Phase 6B adds current HTTP/SOCKS5 dependencies; Phase 6D-A–F adds fail-closed VMess records at top level and through file providers/selectors, including every security label, AEAD framing options, positive legacy AlterID, standard/packet-address/XUDP UDP modes and native TLS/WS/WSS TCP transports | Later protocol records, negative AlterID, advanced TLS/transport options, dialer-proxy chains and their dependency ordering remain |
...
| OUT-07 | VMess: security/alter-id, TCP/UDP, early data and packet modes | Partial: Phase 6D-A–F implements every security label (`auto`, AES-128-GCM, ChaCha20-Poly1305, `none`/`zero`, AES-128-CFB), AEAD padding/authenticated length, zero and positive legacy AlterID headers, native TCP plus standard/packet-address/XUDP UDP, and native TLS/WS/WSS TCP transports with domain/IPv4/IPv6 targets and bounded relay/lifecycle behavior | Negative AlterID, oversized UDP edge cases, packet-address FQDN payloads, UDP over WebSocket, early data, advanced TLS and remaining outer/general-mux transports remain later 6D/7T gates |
