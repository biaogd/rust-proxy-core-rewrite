...
| GRP-02 | URL-test, fallback and load-balance strategies | Partial: fallback, URL-test and all three load-balance strategies have configuration/views, scheduled real HTTP health/delay, failover and TCP routing evidence | 5C1 exhaustive health gates |
| GRP-03 | Group filters, include-all, provider composition, empty fallback, lazy health checks and URL/status policies | Partial: file-provider composition, ordered name/type exclusion, include-all, empty fallback, startup/eager/lazy interval and timeout health scheduling | 5C1 remaining health policies |
