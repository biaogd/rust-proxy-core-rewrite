...
| CFG-03 | Proxies, reserved built-ins, duplicate/reserved-name checks and dependency ordering | Partial: Phase 6A covers reserved/default GLOBAL, collisions and configured direct/reject/DNS/REMATCH validation/order; Phase 6B adds current HTTP/SOCKS5 dependencies; Phase 6D-A–C adds fail-closed native-TCP VMess records at top level and through file providers/selectors, including every AlterID-zero security label and AEAD framing options | Later protocol records, legacy AlterID, transport options, dialer-proxy chains and their dependency ordering remain |
...
| OUT-07 | VMess: security/alter-id, TCP/UDP, early data and packet modes | Partial: Phase 6D-A–C implements every AlterID-zero security label (`auto`, AES-128-GCM, ChaCha20-Poly1305, `none`/`zero`, AES-128-CFB), AEAD padding/authenticated length, native TCP, domain/IPv4/IPv6 targets and bounded relay/lifecycle behavior | UDP/XUDP/packet-address, nonzero AlterID/legacy headers and every TLS/plugin/mux transport remain later 6D/7T gates |
