...
| CFG-03 | Proxies, reserved built-ins, duplicate/reserved-name checks and dependency ordering | Partial: Phase 6A1 covers reserved/default GLOBAL insertion, custom GLOBAL replacement and duplicate/reserved/proxy-group collision rejection | Configured simple adapters and complete dependency-order validation remain |
...
| OUT-02 | REJECT, REJECT-DROP, COMPATIBLE, PASS and PASS-RULE reserved built-ins; configured DNS and REMATCH control adapters | Partial: Phase 6A1 proves current mixed-TCP/control behavior, including the REJECT-DROP hold contract; Phase 5B proves PASS/PASS-RULE/REMATCH rule flow | Configured direct/reject/DNS/REMATCH records, UDP reject behavior and Linux 60-second CI evidence remain |
