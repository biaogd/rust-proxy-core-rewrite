...
| OUT-16 | Mieru, AnyTLS and Sudoku | Partial: Phase 6G-A/B implements Clash `type: anytls` config parse, password auth, native TLS TCP carrier, default padding, session reuse/mux and Go/Rust TCP+mux differentials | UDP/UoT (6G-C), idle/heartbeat recovery polish (6G-D), optional Restls/ShadowTLS/JLS carriers (6G-E), plus Mieru/Sudoku remain open |
