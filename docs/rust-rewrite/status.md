...
| Differential harness | Implemented | All 115 Phase 1–6B Python gates run by default in eight fail-independent GitHub Actions matrix shards; DNS relay, tracked CONNECT and lifecycle-hook readiness use bounded observable retries to tolerate loaded-runner publication latency without normalizing semantic results |
...
no-op and keep cache modification out of that path. A changed ETag validates
and publishes new members, while an over-limit body returns 503 before cache or
configuration mutation. The disabled-switch scenario proves repeated custom
headers remain present while conditional requests remain absent.
...
HTTPS, redirects, provider-aware download routing, global User-Agent behavior,
hashed default paths, durable ETag database interchange, concurrent refresh and
override expressions remain unclaimed.
