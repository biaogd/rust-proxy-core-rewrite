...
| HY2-C Hysteria2 stress/netem/soak | Complete in declared client scope | Concurrent TCP/UDP, cancel churn, authority restart + interrupt recovery, reload proxy-removal (404 + route + RSS/FD bounds), bounded TCP/UDP malformed corpora, fixed-param netem relay (kernel `tc` when available). Diff: `phase_hy2c_hysteria2.py`. Soak harness `phase_hy2c_hysteria2_soak.py` (CI default 45s). **Full ≥2h soak passed** on `8a9b0761` at 2026-09-07T15:29:07Z (`HY2C_SOAK_SECONDS=7200`, rust then go; wall ~4.0h; RSS/FD bounded, failure-rate ok). CI shard `hysteria2` **green** on Linux/Windows/macOS (`1b33e636`). Realm/Gecko/ECH/0-RTT/v1/inbound still deferred |
