...
empty SYNACK for peer v≥2. Runtime owns listen, caps (1024 conns), and TCP →
`serve_shadowsocks_connection`. Evidence: `phase_ing_anytls_tcp.py` (product
AnyTLS outbound vs Go/Rust named inbound). Deferred: UoT UDP, carriers
(ShadowTLS/ResTLS/JLS), ECH/mTLS/`allow-insecure`.
...
inbounds: TCP small/large, UDP multi-dest, wrong-password; Rust-only reject of
combined ws+grpc). WSS half-close remains go↔go-only (same omission as Phase
6F-C); native TLS half-close stays on `phase_inc_trojan_tls.py`.
...
renumber Phase 6 outbound gates). **IN-A is complete as a documentation gate:**
[`inbound-support-matrix.md`](inbound-support-matrix.md) records Go named
listeners from `listener/parse.go`, Rust fixed HTTP/SOCKS/mixed + Phase 6C-N
...
Snell AEAD `read_plain` now advances through the cancel-safe `read_phase`
state machine, so a `select!` that drops a mid-record UDP `recv` no longer
loses salt/length/ciphertext bytes. `poll_write` / `poll_flush` /
`poll_shutdown` propagate `flush_pending` errors and mark the stream failed
so torn ciphertext cannot return to the v2 reuse pool. Product Snell UDP
sessions keep `send` inside a cancellable `select!` with shutdown, tracker
cancel, and the idle timeout (half-written associations are dropped). The
controller keeps successful 0ms health samples (aligned with PR #19 / Go)
instead of flooring probes to 1ms. Branch also merges `codex/rust-rewrite`
after SSH PR #19. Evidence: `aead` cancel/write-failure unit tests plus
existing `tcp_relay` / `udp_relay` and `phase7e_snell_*.py`. Not Parity.
...
Userspace WireGuard outbound now recovers a restarted peer with one TCP
timeout plus forced handshake, sends `persistent-keepalive` while idle,
re-resolves `refresh-server-ip-interval`, installs TUN loop-avoidance for the
peer endpoint, and tears down mixed UDP sessions on network-generation bumps.
Evidence: `compat/scripts/phase6i_wireguard_lifecycle.py` plus
`protocol-wireguard` `lifecycle`. AmneziaWG, `peers`, `ip-stack` and inbound
remain rejected. Not Parity; three-platform CI is the existing wireguard
shard, not a native Parity claim.
...
Not Parity; three-platform CI remains open.
...
plus `protocol-wireguard` `tcp_relay`. Not Parity; three-platform CI remains
open.
...
fixture files. Darwin system DNS apply still rewrites primary-service
`ServerAddresses`; the TUN DNS next address also gets an owned `/32` on the
utun because `associate_route(false)` does not install that point-to-point
route. The 8B native gate resolves `http.phase8b.example.com` through
getaddrinfo (not `.test`, which current macOS intercepts as mDNS). Still
Partial, not Parity; 8B/8C native remain unclaimed; not merge-ready.
...
socket (listeners, Shadowsocks, controllers, DNS) **before** restoring retired
binds, then republish the previous config when it was already swapped in.
Prepare-phase `?` failures after stopping a conflicting old listener use that
same path, so a same-port SS/mixed replacement followed by TUN start failure,
or a bind to an unassigned address after the old port was closed, restores the
old port and releases the new one. 8A native adds those two cases. 8C Windows
staging points Rust at the scratch `wintun.dll` via `MIHOMO_WINTUN` and skips
a WinError 32 copy onto an already-loaded next-to-exe file. Still Partial, not
Parity; 8B/8C/8F native remain unclaimed; not merge-ready.
...
local `:443`. Network-change apply updates per-family auto-detect bind
names and **retires cached TUIC/Hysteria2 clients** so QUIC Endpoints rebind
the new NIC; client cache identity includes those resolved binds. Hysteria2
(plain, Salamander, and port-hop) uses `bind_outbound_udp` like TUIC. The 8F
dual-uplink fixture now takes the old veth down after the flap. Clippy:
`is_global_unicast` is Linux/Android-only; `DnsSnapshot` dead_code allow
covers the unused Darwin/Windows variant. Not merge-ready; not Parity.

### Gun cancellation isolation correction — 2026-09-09

Cancelling a pending Gun response-header wait now releases only that request's
active lease. It no longer closes the shared HTTP/2 connection. The h2 library
resets the abandoned logical stream; physical connection failures remain owned
by the connection driver. A deterministic two-stream unit test checks CANCEL,
active counts, continued echo and subsequent reuse of the same connection.
`phase6d_vmess_udp_carriers.py` additionally stalls only the second stream's
response headers and requires the first association to keep echoing over exactly
one physical connection in both Go and Rust. Local macOS arm64 verification
passed: all six Gun unit tests (including the two cancellation regressions), the 12 carrier combinations, immediate
and early-data WS timeout/shutdown checks, and pooled Gun cancellation isolation.
Format and all-target/all-feature Clippy passed. Full workspace tests compiled,
but the run was stopped after repeated executable-startup stalls, including with
temporary local staging; no full-workspace test pass is claimed for this change.
The immediate WS disconnect matches Go; the bounded deferred early-data
disconnect remains an explicitly Rust-only safety contract. Native CI is pending.
...
with Go's quic-go Cubic/BBR is not claimed. `disable-sni` now clears rustls
`enable_sni` (certificate skip still follows Go `skip-cert-verify || disable-sni`);
explicit `alpn: []` is preserved; v5 does not apply YAML `request-timeout` to
stream open; legacy `recv-window` / `recv-window-conn` follow Go (conn→stream,
window→connection). v4, 0-RTT, inbound, ECH and UDP-over-stream remain rejected
or unimplemented.
...
**TUN Phase 8 is in progress: Linux 8A, Darwin 8B, and Windows 8C are
implemented in this checkout.** SSR and TUIC v5 outbound
...
runtime enablement for linux|macos aarch64. 8C reuses that data path on
...
physical default (TUN excluded), loop-avoidance re-protect **via that snapshot**
...
DIRECT UDP sessions rebound via a network-generation watch, loop-avoidance
host routes using the matching address-family default (uncaptured IPv6 is
skipped instead of `IPv6/128 via IPv4-gateway`), auto-detect bind choosing
the IPv4 or IPv6 physical NIC from the remote family (pure IPv6 is not
`is_lost()`; a captured family with no physical egress refuses outbound
instead of leaking into TUN), `bind_outbound_udp` taking
separate local/remote addresses so loopback remotes skip NIC bind,
...
| Phase 8B Darwin arm64 TUN | In progress | Reuses the 8A data path. utun open, Darwin split auto-route, scutil DNS ownership/restore, TUN-DNS `/32` on utun, `tun0`/`utun` rejected without remap. Unprivileged identity in `phase8b_tun.py`; privileged `PHASE8B_NATIVE=1` fail-closed Darwin arm64 harness is not Parity until `phase8b-darwin-tun` reports success |
...
| IN-E VMess TLS inbound | Complete (declared TLS scope) | Named TLS TCP+standard UDP; AEAD alterId 0; `phase_ine_vmess_tls.py` |
...
## Phase 6C-M3 Shadowsocks v2ray-plugin WebSocket evidence

The product side uses `tokio-tungstenite` rather than hand-written handshake
or frame parsing. Axum itself is server-oriented, so the independent authority
uses Axum's `WebSocketUpgrade` and wraps its binary message stream as the byte
transport consumed by the official Shadowsocks server. Product routing and
plugin policy remain outside the WebSocket adapter.

`compat/scripts/phase6c_shadowsocks_v2ray_websocket.py` passes on Darwin arm64,
2026-08-29. With `mux: false`, it verifies the configured Host and path at the
Axum boundary and compares domain TCP, 128 KiB IPv4 TCP, process survival and
the pinned Go client's non-preserved half-close. The Rust scope test explicitly
rejects default/enabled mux and TLS instead of claiming the broader options.
Linux execution is configured in the default controller/outbound Actions
shard. Headers, TLS, mux, early data, raw HTTP-upgrade, provider/plugin
combinations and server direction remain open.

## Phase 6C-M4 Shadowsocks v2ray-plugin WebSocket TLS evidence

The M3 client now composes the existing Rustls verification boundary before
the Tungstenite handshake. This keeps certificates, native/global roots,
adjusted time and verification bypass outside WebSocket framing. The Axum
authority uses a custom Axum `Listener` that accepts either raw TCP or a
Tokio-Rustls stream, so plaintext and WSS evidence share the exact same Axum
Upgrade and official Shadowsocks server path.

`compat/scripts/phase6c_shadowsocks_v2ray_websocket_tls.py` passes on Darwin
arm64, 2026-08-29. It proves the fixture root in
`tls.custom-certifactes` validates `dot.phase4.test`, an absent root is rejected
when verification is enabled, and `skip-cert-verify: true` deliberately
bypasses that rejection. The trusted path also passes exact SNI/Host/path,
domain TCP, 128 KiB IPv4 TCP, process survival and the pinned Go half-close
...
## Phase 6E-C VLESS WebSocket/WSS evidence

Phase 6E-C composes the Phase 6E-A stream over the existing VMess-style
WebSocket carrier from `rewrite-transport`. Config accepts `network: ws` with
`ws-opts.path` and `ws-opts.headers` only; early-data and raw HTTP Upgrade
options remain rejected until a later gate.

```bash
PHASE6EVLESSWS_CARGO_TARGET=/path/to/target python3 compat/scripts/phase6e_vless_websocket.py
```

The gate exercises plaintext WS and TLS WSS through mixed/rule/group/provider
routing, custom headers, trusted and name-override TLS, skip/untrusted
rejection, first-packet readiness, 128 KiB relay, half-close and process
survival. HTTP/H2/gRPC/xHTTP, UDP/XUDP, Vision, Reality, encryption and server
direction are not claimed by Phase 6E-C.

## Phase 6E-D VLESS HTTP/H2 evidence

Phase 6E-D composes the Phase 6E-A stream over the existing VMess-style HTTP/1 and
HTTP/2 carriers from `rewrite-transport`. Config accepts `network: http` with
`http-opts` and `network: h2` with `h2-opts` only; cross-network options remain
rejected until a later gate.

```bash
PHASE6EVLESSHTTP_CARGO_TARGET=/path/to/target python3 compat/scripts/phase6e_vless_http.py
```

The gate exercises plaintext HTTP/1, TLS HTTP/1, plaintext H2 and TLS H2 through
mixed/rule routing, custom headers, default option fallbacks, first-packet
readiness, 128 KiB relay, H2 half-close rejection and process survival.
gRPC/xHTTP, UDP/XUDP, Vision, Reality, encryption and server direction are not
claimed by Phase 6E-D.

## Phase 6E-E VLESS gRPC/Gun evidence

Phase 6E-E composes the Phase 6E-A stream over the existing VMess-style single-stream
Gun carrier from `rewrite-transport`. Config accepts `network: grpc` with
`grpc-opts.grpc-service-name` and `grpc-opts.grpc-user-agent` only; pooled gRPC
options remain rejected until a later gate.

```bash
PHASE6EVLESSGRPC_CARGO_TARGET=/path/to/target python3 compat/scripts/phase6e_vless_grpc.py
```
...
## Phase 6E-G VLESS Vision evidence

Phase 6E-G adds `flow: xtls-rprx-vision` on native TCP with TLS. Config accepts only
`xtls-rprx-vision` together with `tls: true` and `network: tcp`; flow with UDP or
without TLS is rejected in Rust. `rewrite-protocol-vless` encodes the flow protobuf
addon and wraps the VLESS stream with Vision padding framing aligned to Mihomo's
client path.
...
- Soak harness: `compat/scripts/phase_hy2c_hysteria2_soak.py`. CI uses
  ...
  `HY2C_SOAK_SECONDS=7200` (runs rust then go for that duration each).
- **Full soak evidence (2026-09-07):** SHA `8a9b0761`, started
  `2026-09-07T11:29:01Z`, finished `2026-09-07T15:29:07Z` (wall ~4.002h),
  exit 0. Outcome: `full-soak=true`, `failure-rate-class=ok`,
  `rss-bounded=true`, `fd-bounded=true`, `process-alive=true`. Mid-run
  samples: rust client ~51 MiB RSS / 15 FDs at ~68 min; go client ~52 MiB
  RSS / 10 FDs early in its 2h window. No failure artifact; stderr only
  showed the Cargo finish line.
- CI: new `hysteria2` differential shard runs `phase_hy2a_*`, `phase_hy2b_*`,
  `phase_hy2c_*` on Linux x86_64, Windows x86_64, macOS arm64 — **green** on
  all three at `1b33e636` (after wait_exchange / TCP-UDP retry / netem
  recovery flake hardens).
