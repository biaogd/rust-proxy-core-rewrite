...
| Phase 6C-N Shadowsocks ss-config inbound | Complete in declared v3 TCP scope | Named `listeners` SS inbound with shadow-tls v3 passes Go/Rust differential for authenticated TCP, plain-TLS fallback relay to `handshake.dest`, concurrent accept while fallback holds, `IN-USER` routing via `metadata.inbound_user`, config rejection of inbound v1/v2 and per-connection ShadowTLS accept before the 10s SS handshake timeout; `handshake.proxy` uses the same rules/group outbound path as inner TCP (empty proxy follows normal routing). Fallback relay uses bidirectional copy for half-close |
