...
| VLESS | **Partial — IN-D TLS + Vision + WSS + gRPC + XUDP + REALITY + Vision+REALITY** | `named_listeners.rs`, `runtime/vless_listener.rs`, `protocol-vless::server`, `transport` `accept_vision_tls` / `accept_reality` / `accept_reality_vision` / `accept_websocket_path` / `V2rayGrpcServerConnection` | Yes | Standard-mode UDP on TLS; Mux/XUDP multi-dest on TLS; Vision TCP on native TLS and REALITY; REALITY TCP auth Accept | UUID users + optional `flow: xtls-rprx-vision` (TLS or REALITY native TCP) | Native TLS; REALITY via `reality-config`; WS via `ws-path`; Gun via `grpc-service-name` | Yes | Named `type: vless`; combined ws+grpc rejected; REALITY dest fallback / xHTTP deferred |
