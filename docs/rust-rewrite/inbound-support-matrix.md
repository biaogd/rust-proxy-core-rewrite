...
| VLESS | **Partial — IN-D TLS** | `named_listeners.rs`, `runtime/vless_listener.rs`, `protocol-vless::server` | Yes | Standard-mode UDP (fixed dest) | UUID users | Native TLS only | Yes | Named `type: vless`; `ws-path` / `grpc-service-name` / `reality-config` / per-user `flow` rejected |
...
| Inbound Reality / ReSTLS / JLS / WS / H2 / gRPC / xHTTP / mKCP / Mekya / mux | Missing as inbound | — | — | — | — | ShadowTLS v3 on SS; Trojan TLS + WS + Gun; VLESS TLS (no shared mux / Reality / Vision) | — | `IN-13` |
...
| IN-08 | VMess + VLESS server | VLESS **Partial — IN-D** TLS TCP+standard UDP; VMess not started | **IN-D** (VLESS TLS done; WS/gRPC/Vision/REALITY open), **IN-E** (VMess) |
