...
| Trojan | **Partial — IN-C TLS + WSS + gRPC + REALITY** | `named_listeners.rs`, `runtime/trojan_listener.rs`, `protocol-trojan`, `transport` `accept_websocket_path` / `V2rayGrpcServerConnection` / `accept_reality` | Yes | UDP-over-TLS | Password users (SHA-224) | Native TLS; REALITY via `reality-config`; WS via `ws-path`; Gun via `grpc-service-name` | Yes | Named `type: trojan`; combined ws+grpc / `ss-option` rejected; REALITY dest fallback deferred |
...
| VMess | **Partial — IN-E TLS + WSS + gRPC + XUDP + REALITY** | `named_listeners.rs`, `runtime/vmess_listener.rs`, `protocol-vmess` AEAD Accept + XUDP framing | Yes | Standard body-record UDP on TLS; Mux/XUDP multi-dest on TLS | UUID users; `alterId` must be 0/absent | Native TLS; REALITY via `reality-config`; WS via `ws-path`; Gun via `grpc-service-name` | Yes | Named `type: vmess`; combined ws+grpc / nonzero alterId / mKCP/Mekya rejected; REALITY dest fallback deferred |
...
`ListenerKind` today: `Http | Socks | Mixed | Redir | Tproxy | Tunnel | Shadowsocks | Trojan | Vless | Vmess | Hysteria2 | Tuic | AnyTls`
(`rust/crates/config/src/model.rs`). `InboundProtocol` today:
`Http | Https | Socks4 | Socks5 | Shadowsocks | Trojan | Vless | Vmess | Hysteria2 | Tuic | AnyTls | Tun | Inner | Redir | Tproxy | Tunnel`.
...
| IN-08 | VMess + VLESS server | VLESS **Partial — IN-D** TLS+WSS+gRPC+XUDP+Vision+REALITY; VMess **Partial — IN-E** TLS+WSS+gRPC+XUDP+REALITY (AEAD alterId 0) | **IN-D** / **IN-E** (declared carriers done; Reality dest fallback / legacy alterId / plain TCP / mKCP open) |
